...
| 20 | `docs/components/20_schluessel_tresor.md` | Schlüssel-Tresor · **Code-Stub** (Spec PR #315 + Bau-Sitzung 2026-06-20). `src/modules/20_schluessel_tresor.js` voll angelegt: lokal verschlüsselter Tresor für die SBKIM-Identität (nodeId + privater Schlüssel + Spore), Krypto-Kern wiederverwendet Modul 02 `exportBackup`/`importBackup` (PBKDF2-SHA256 ≥600k + AES-GCM-256), Store `sbkim_vault` (Modul 01 ensureStore). **Shamir-Recovery 2 von 3 über das Passwort** (GF(256), `recoverPassword`). Auto-Abfrage-Modal (zweistufiges Passwort + Anteile-Sicherungs-Bestätigung). Schnittstelle `SbkimVault = {init/hasVault/isUnlocked/createVault/unlock/lock/recoverPassword/_meta}`. Headless-Smoke `tests/smoke_bau20_tresor.mjs` **19/19 grün**; `index.html` lädt das Skript (KEIN Auto-Init — kein Überraschungs-Prompt), Panel 20 in `manual_check.html`. **Browser-Sichttest der Modal-UI + Sage-Page-Auto-Prompt-Wiring ausstehend.** Datenschutz: nur Identität/Schlüssel, kein PII, nie übers Netz. |
