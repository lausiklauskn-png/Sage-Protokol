...
<div class="mk-leiste" data-mk-leiste role="toolbar" aria-label="Markieren">
  <button type="button" data-mk-tun="gruen" title="gut so — bleibt">&#129001;</button>
  <button type="button" data-mk-tun="gelb"  title="unklar — nachfragen">&#129000;</button>
  <button type="button" data-mk-tun="rot"   title="ändern — muss überarbeitet werden">&#128997;</button>
  <button type="button" data-mk-tun="notiz" title="Notiz dazuschreiben">&#9998;</button>
  <button type="button" data-mk-tun="weg"   title="Markierung entfernen">&#10005;</button>
</div>
...
(function(){
  ...
  var FARBEN = {
    gruen: { zeichen: "\u{1F7E9}", wort: "gut so",  sinn: "bleibt, wie es ist" },
    gelb:  { zeichen: "\u{1F7E8}", wort: "unklar",  sinn: "nachfragen oder pruefen" },
    rot:   { zeichen: "\u{1F7E5}", wort: "aendern", sinn: "muss ueberarbeitet werden" }
  };
  ...
  function bericht(){
    ...
    for(var q in nachQuelle){
      ...
      for(var f = 0; f < reihe.length; f++){
        ...
        out.push("### " + FARBEN[farbe].zeichen + " " + farbe.toUpperCase()
          + " — " + FARBEN[farbe].sinn + " (" + stueck.length + ")");
        out.push("");
  ...
  function tafelZeichnen(){
    ...
    if(zaehler){
      zaehler.textContent = marken.length === 0 ? "noch keine"
        : (marken.length + " insgesamt · "
           + FARBEN.gruen.zeichen + " " + z.gruen + " · "
           + FARBEN.gelb.zeichen + " " + z.gelb + " · "
           + FARBEN.rot.zeichen + " " + z.rot);
