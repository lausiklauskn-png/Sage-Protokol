# Brief — Re-Aktivierung Modul 16 Sub (e) in Mein-Rezeptbuch (extern)
...
Diese Bau-Sitzung zieht den neuen Stand in Mein-Rezeptbuch nach und
beweist live (Klaus' Browser-Sichttest), dass der Bronze→Gold-
Wechsel beim ersten Cross-Knoten-Handshake mit Mein-Mixarium
stattfindet.
...
- PR #180 + #181 sind in Sage-Protokol auf `main` (✅ erledigt
  2026-05-26).
- Mein-Rezeptbuch hat bereits Modul 15 + 16 + 17 Floating-Widget
...
**Branch-Vorschlag (im Endknoten-Repo):** `claude/migration-rezeptbuch-reaktivierung`
...
Branch: claude/migration-rezeptbuch-reaktivierung (vom main aus
...
A. **`sbkim/16_siegel.js` ersetzen** durch die exakte Sage-Datei vom
   ...
   trägt (z.B. „Rezeptbuch-spezifisches Sicherheits-Update"), diese
...
B. **`index.html` CSS um Sub-(e)-Block erweitern.** Im bestehenden
   ...
   Falls Mein-Rezeptbuch sein CSS in einer separaten Datei führt
...
E. **Tests im Endknoten (Browser-Sichttest):**
   ...
   Mein-Mixarium in Tab 2 derselben Chrome-Instanz öffnen
   ...
   2. **Cross-Knoten-Handshake auslösen.** In Mein-Mixarium-Tab
      via Eruda-Konsole:
      ```js
   ...
   3. **Im Mein-Rezeptbuch-Tab:** Der SIEGEL-Slot im Widget muss
...
- Commit + Push auf `claude/migration-rezeptbuch-reaktivierung`.
...
- **Parallel:** Bau-Sitzung in Mein-Mixarium (siehe
  `BRIEF_REAKTIVIERUNG_ENDKNOTEN_MM.md`). Erst wenn beide laufen,
  ist der bidirektionale Bronze→Gold-Wechsel beweisbar.
- **Cross-Knoten-Bonus-Sichttest:** Klaus tippt in MR, sieht
  Treffer aus MM (über BRIEF_BAU_ENDKNOTEN_SUCHFELD_MR.md — wenn
...
1. Beide PWAs öffnen. Beide SIEGEL-Slots im Widget sind initial
   ...
   Block.
2. In MR Eruda: SbkimAnastomose.handshake(peerSpore, ownVec) gegen
   MM. Outcome "established".
3. In MR Tab: SIEGEL wechselt auf Gold (Stufenwechsel-Animation
   600 ms beobachtbar). Modal-Klick zeigt voll-Aspekte-Liste mit
   Aspekt 4 datiert.
4. In MM Tab: BroadcastChannel-Bridge routet den Empfänger-Event
   durch — MM-SIEGEL wechselt parallel auf Gold.
5. Tab-Reload in MR: SIEGEL wieder Bronze (RAM-only).
6. Tab-Reload in MM: SIEGEL wieder Bronze (RAM-only). Beide
