...
| Bau 07.Y transparenter Slot-Pfad + `_sendLegacyForIdentity`-Hook | 2026-05-20 | Bau 07.Y | **Code in `src/modules/07_apoptose.js` additiv-mit-internem-Refactoring** (keine äußere Signatur-Änderung außer optionalen `key`-Parametern auf `listLegacy(key?)` und `forgetExpiredSiblings(maxAgeMs, key?)`, beide rückwärtskompatibel). Drei Eingriffe: (1) transparenter Slot-Pfad in Storage-Stores; (2) globale `confirmSelfApoptose` iteriert über ALLE Slots; (3) neuer interner Hook `_sendLegacyForIdentity(key, reason?)`, den Bau 02.Y bereits via typeof-check fail-soft ruft. Modul-lokale Konstanten `SIBLINGS_STORE` / `LOG_STORE` / `INBOX_STORE` / `HETERO_INBOX_STORE` durch `_BASE`-Variante ersetzt; `HETERO_OUTBOX_STORE_BASE = "sbkim_hetero_outbox"` neu (für globalen Cleanup-Pfad); `DEFAULT_IDENTITY_KEY = "main"` als Fallback. `CLEANUP_ORDER` durch `CLEANUP_ORDER_BASES = [siblings, log, legacy_inbox, hetero_inbox, hetero_outbox]` ersetzt (slot-suffixed pro Slot durchlaufen). Fünf neue Closure-Helper für Store-Namen + `ensureSlotStores(slot)` (idempotent dank Bau-01.Y). Modul-State um `activeSlotKey` + `receiverMap` + `ownPrivateKeyCacheBySlot` erweitert. `init()` ruft `getOrCreateIdentity()` + cached `activeSlotKey` + `ensureSlotStores` + baut `receiverMap` über alle Slots. **`prepareSelfApoptose(reason)`**: `recipientCount` zählt über ALLE Slots. **`confirmSelfApoptose(token, reason)` komplett umgeschrieben**: iteriert `slots = listIdentities()`; PRO Slot `await _sendLegacyForIdentity(slot, reason)` (fail-soft, aggregiert recipientsNotified + recipientsFailed); danach PRO Slot Cleanup über `CLEANUP_ORDER_BASES` + `del(SPORE_STORE, slot)` + `del(KEYS_STORE, slot)`; danach globaler `del(sbkim_meta, "active-identity")`; Closure-Caches invalidiert + `SbkimSpore.resetIdentityCache()`. **`_sendLegacyForIdentity(key, reason?)` neu**: Aufrufer (a) Bau 02.Y `removeIdentity(key, {force:true})` via typeof-check; (b) `confirmSelfApoptose` iteriert. Sendet Persona-Vermächtnis an Geschwister DIESER Persona; signiert PRO Sibling separat mit `toNodeId: sibling.nodeId`; KEIN Store-Cleanup. Fail-soft: returns `{recipientsNotified, recipientsFailed}` (Default-Reason `"Persona-Apoptose (slot=<key>)"`). **Auf `window.SbkimApoptose._sendLegacyForIdentity` exportiert** (Modul 02's typeof-check sucht da). **`receiveLegacy(legacyMessage)`**: Receiver-Map-Lookup auf `toNodeId`: in Map → targetSlot als Persona; nicht in Map → `rejected, reason:"toNodeId stimmt nicht"`, KEIN Storage-Eingriff; toNodeId fehlt → activeSlotKey-Fallback. Inbox-Schreib in `legacyInboxStoreName(targetSlot)`, Sibling-del aus `siblingsStoreName(targetSlot)`, Response signiert mit getroffener Persona. **`listLegacy(key?)`** + **`forgetExpiredSiblings(maxAgeMs, key?)`** um optionalen key-Parameter erweitert. `loadOwnPrivateKey(slotKey?)` lädt pro Slot, cached in `Map<slotKey, CryptoKey>`. Selbstcheck-Zeile UNVERÄNDERT. `_meta` um Basis-Namen + Getter `activeSlotKey` + Getter `receiverMapSize` erweitert. Test-Brücke `_buildSignedLegacyMessage(reason)` signiert mit aktiver Identität. Modul-Kopfkommentar um Bau-07.Y-Block am Ende. `node --check` grün. Headless-Smoke-Test `tests/smoke_bau07y_transparent_slot_pfad_und_legacy_hook.mjs`: 30 Sub-Proben, 30 grün. Regression-Smokes alle weiterhin grün. **Konsumenten-Achse 05/06/07/08 jetzt vollständig slot-suffixed.** **Bau-02.Y-fail-soft-Klausel aufgelöst:** Modul 02's typeof-check findet den Hook jetzt; `console.warn`-Pfad verschwindet automatisch ohne Modul-02-Code-Änderung. Migrations-Hinweis: alte `sbkim_legacy_inbox`-Daten via `SbkimSpore.importBackup` (Bau 02.Y) nach `sbkim_legacy_inbox_main` bringen. `PROTOCOL_VERSION` / `DB_VERSION` / `BACKUP_FORMAT_VERSION` unverändert. KEIN Modul-01/02/03/04/05/06/08-Eingriff. `status.json` unverändert (Modul 07 bleibt `score:"fertig"`). **Sichttest 2026-05-20 grundbelegt** (Klaus, DeX-Chrome auf Galaxy Tab S6): Panel 07 Test 6 Self-Apoptose IRREVERSIBEL live grün — `confirm_outcome:"completed"`, `recipients_notified:[]`, `recipients_failed:[2 Pseudo-Endpoints]` (erwartet — `http://127.0.0.1:1/...` führt ins Leere; fail-soft korrekt), `stores_alle_leer:true`, `getNodeId_wirft_NoIdentityError:true`. Globale Slot-Iteration über alle Slots im Cleanup-Pfad bestätigt (alle identitäts-spezifischen Stores geleert + globaler `sbkim_meta["active-identity"]`-Reset + Modul-02-Cache-Invalidate). `_sendLegacyForIdentity`-Hook produktiv (Bau-02.Y-fail-soft-Klausel aufgelöst — der Hook wird pro Sibling separat mit `toNodeId: sibling.nodeId` signiert und gesendet). Voller Test-1-bis-8-Lauf + Panel 02 Knopf 9 Persona-Apoptose-Hook ohne `console.warn` steht noch aus (Klaus' Wahl). |
