...
confirmSelfApoptose(token, reason) → Promise<{outcome, recipientsNotified, recipientsFailed}>
  ...
  //        clear(sbkim_legacy_inbox) → clear(sbkim_spore) →
  //        clear(sbkim_keys) → SbkimSpore.resetIdentityCache().
