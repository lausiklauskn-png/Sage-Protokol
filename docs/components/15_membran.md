...
| Sichttest | ungeprüft 2026-05-24 | Bau-Sitzung 15 — wartet auf Klaus' Browser-Lauf | Headless-Smoke 11/11 grün (Node mit DOM-Stub). Browser-Sichttest wartet auf Klaus: Panel 15 Setup + Tests 1–7 in `tests/manual_check.html` (DeX-Chrome auf Galaxy Tab S6) sowie Sage-Page-Sichttest in `index.html` (Navleisten-Lampe `#lamp-fremd` da → Klick öffnet Modal → `SbkimMembrane.fremdzugriff._recordForTest(...)` aus Konsole/Eruda → Lampe rot + Modal-Tabelle füllt sich → Aufräumen-Knopf leert Tabelle + nimmt Lampe aus). **Bau 15.SW SW-Probe Sichttest ausstehend** — Knopf 8 in Panel 15 prüft den BroadcastChannel-Pfad headless (End-to-End SW-→Page-Empfang ohne SW-Lauf, simuliert via direktem `new BroadcastChannel("sbkim-membrane").postMessage(...)`), Knopf 9 ist nur ein Klaus-Hinweis für echten Cross-Origin-Test (nicht reproduzierbar in `manual_check.html` allein — braucht zwei Origins). |
