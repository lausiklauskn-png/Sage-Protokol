...
| Pflege Sub-(e)-Visueller-Slot-Render | 2026-05-26 | Pflege 17 Widget-Bronze/Gold-Render | Folge-Pflege zum Endknoten-Sichttest Cross-Knoten Sub (e) 2026-05-26 (Sub-(e)-Folge-Befund 1): Modul 17 rendert SIEGEL-Slot-Button stufen-unabhängig als Gold-Medaillon — Modul 16's `data-stufe="bronze"`/`"gold"` wirkt nur am unsichtbaren Widget-Proxy-Span im Inneren, NICHT am sichtbaren Slot-Button. Klaus' Befund DeX-Chrome: visueller Bronze/Gold-Unterschied im Slot fehlt, Stufe nur im Modal erkennbar. **Eingriff additiv** (KEIN Bruch der bestehenden Public-Surface): neuer Closure-State `siegelStufeRendered` (`"bronze"` \| `"gold"` \| `null`), neuer Helper `getCurrentSiegelStufe()` (fail-soft Lookup auf `SbkimSiegel._meta.siegelStufe`, Default `"bronze"`), neuer Helper `applySiegelStufe(stufe, withAnimation)` (setzt `data-siegel-stufe`-Attribut am sichtbaren Slot-Button + idempotent, Animation nur bei `"bronze"`→`"gold"`-Transition). `mountSiegelSlot()` ruft `applySiegelStufe(getCurrentSiegelStufe(), false)` nach Slot-Mount (initial-Stufe). `onHandshake()` erweitert um `setTimeout(0) { applySiegelStufe(getCurrentSiegelStufe(), true) }` — der setTimeout(0)-Trick verschiebt den Lookup ans Ende des Event-Loops, damit Modul 16's Listener garantiert gelaufen ist (Listener-Reihenfolge auf `addEventListener` ist Registrierungs-Reihenfolge; Modul 17 init läuft VOR Modul 16 init im Endknoten-Andocker). **CSS-Block erweitert** um vier neue Regeln (`[data-siegel-stufe="bronze"]::before` + Glyph mit `filter:saturate(0.5) brightness(0.78)`, `[data-siegel-stufe="gold"]::before` + Glyph mit `filter:none`, `.sbkim-widget-siegel-stufenwechsel::before` mit Animation) + neuer Keyframe `sbkim-widget-siegel-stufenwechsel` (600 ms Skalierungs-Puls 1.0→1.35→1.0 mit Gold-Glow-Box-Shadow). `_meta` erweitert um Live-Getter `siegelStufeRendered`. **Panel 17** in `tests/manual_check.html` um zwei Knöpfe 13+14 erweitert (Test 13 Bronze-Initial-Stand am Slot, Test 14 synthetischer Handshake → Gold-Wechsel + Stufenwechsel-Klasse). **Headless-Smoke** `tests/smoke_bau17_floating_widget.mjs` von 32 → **34 Proben** (zwei neue Proben 32+33 für Stufen-Render-Logik), 34/34 grün. **Regression** smoke_bau15b 31/31 + smoke_bau16_sub_e_bronze 15/15 grün. `node --check` für `src/modules/17_floating_widget.js` + alle 13 Inline-`<script>`-Blöcke in `tests/manual_check.html` grün. **KEIN Modul-16-Eingriff** (Modul 16's Sub-(e)-Logik bleibt unangetastet — Modul 17 spiegelt nur den Stand am sichtbaren Slot), **KEIN PROTOCOL_VERSION-/DB_VERSION-/BACKUP_FORMAT_VERSION-Bump**, **KEIN ZERTIFIKAT_ASPEKTE-Eintrag** (Render-Schicht-Pflege, kein Sicherheits-Modul-Update), **KEIN Endknoten-Eingriff** (Endknoten ziehen den neuen Modul-17-Code in eigener Folge-PR nach), **KEINE Tafel-Umsortierung CLAUDE.md**. Brief: `docs/sessions/BRIEF_PFLEGE_17_WIDGET_BRONZE_GOLD_RENDER.md`. |
