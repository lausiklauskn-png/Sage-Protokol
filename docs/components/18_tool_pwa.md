...
4. **Handshake** (`SbkimAnastomose.handshake(foreignSpore,
   ownDomainVector)`). **Pflicht:** `handshake` erwartet als 2. Argument
   einen eigenen Domain-Vektor als `Float32Array(384)` (Modul 05
   § `_doHandshake` `ownDomainVector`-Check). Modul 18 leitet ihn
   kanonisch ab — `SbkimEmbedding.embedPassage(domainKeywords.join(", "))`
   — analog zur Spore-`domainVector`-Erzeugung (siehe
   `tests/manual_check.html` `mainVec`). Wer nur
   `handshake(foreignSpore)` ruft, bekommt sofort „ownDomainVector muss
   Float32Array(384) sein — Aufruf von handshake".
