...
| Bau 04.F Hybrid BM25+Vektor | 2026-07-01 | Bau-Sitzung Semantik-Matching Strang A1 (Brief 2026-07-01) | **Code in `src/modules/04_match.js` additiv** (keine bestehende Funktion verändert, DEFAULT-Pfad byte-gleich). STRANG A1 aus dem RAG-Werkzeugkiste-Brief: **BM25** (lokaler, offline, deterministischer lexikalischer Score) + **Reciprocal Rank Fusion (RRF)** ergänzen den reinen e5-Cosinus, dessen Anisotropie-Boden (~0.82) Bedeutung nicht zuverlässig trennt (siehe `LEHRE-EMBEDDING-MATCH-KALIBRIERUNG.md`). **Drei neue Funktionen:** `tokenizeBM25(text)` (unicode-bewusst, lowercase, sprach-agnostisch), `bm25Scores(queryText, docTexts, options?)` (Robertson-Sparck-Jones-IDF, `k1=1.5`/`b=0.75`, deterministisch, exportiert für Panel-04-Messung), interner `rrfScore(vRank, lexRank)` (`RRF_K=60`). **`queryLocal` erweitert um opt-in `options.hybrid`:** ohne Flag byte-gleiches Bau-04.C-Verhalten (nur Cosinus, Filter ≥ `PROVIDER_MIN_MATCH`); mit `hybrid:true` fusioniert es BM25+Vektor via RRF — Aufnahme = (cos ≥ 0.80 **ODER** bm25 > 0), Sortierung nach `fused` (Tie-Break Cosinus). Treffer tragen additiv `bm25` + `fused`; `score` bleibt der Cosinus (unveränderte Semantik). **Kern-Hebel:** ein Eintrag UNTER dem Vektor-Boden mit exaktem Wort-Treffer wird jetzt gefunden (den der reine Cosinus verwarf). **Korpus-Schema additiv erweitert:** optionales `text`-Feld (roher Passage-Text für BM25; fehlt es → Fallback auf `label`); `validateCorpus` prüft `text` nur wenn vorhanden — Bestands-Korpora ohne `text` bleiben gültig. **`_meta`** um `bm25K1`/`bm25B`/`rrfK` + `hybridQueryLocalNote` erweitert; Selbstcheck-Zeile nennt `bm25Scores`. **Headless-Smoke** `tests/smoke_bau04f_hybrid_bm25.mjs`: **32 Sub-Proben, 32 grün, 0 rot** (Export-Anker, Tokenizer, BM25-Grundlagen + IDF-Effekt, DEFAULT-Regression, Kern-Hebel unter-Boden-Lexik-Treffer, Fusion-Reihenfolge, text-Fallback, leerer Korpus/k-Grenze, validateCorpus text-Typ). Regression: smoke_bau04c 43/43, smoke_bau04d 68/68, smoke_bau04e 29/29 grün. `node --check` grün. **TABU eingehalten:** `PROVIDER_MIN_MATCH` (0.80) unverändert = Vektor-Pfad-Boden UND Andock-Riegel (Modul 05 unberührt); KEIN Netz/LLM/Schlüssel in BM25; KEIN PROTOCOL_VERSION-/DB_VERSION-Bump; KEIN Modul-Eingriff außer 04. **Panel-04-Messung (Schritt 0) + Browser-Sichttest wartet auf Klaus.** |
| Sichttest (Bau 04.F) | 2026-07-01 | Klaus (Termux, `python3 -m http.server`) | **✅ GRÜN** — Panel 04 **Test 20** „Hybrid BM25+Vektor" live bestätigt. Status-Chip „Hybrid OK (A unter Boden wird gefunden)". Frage „wespen hausmittel": **Standard (nur Cosinus)** liefert nur B (0.8645) + C (0.8369) — „A (unter Boden)" (cos 0.7091) fehlt. **Hybrid** stellt „A (unter Boden)" mit `bm25 1.3938`/`fused 0.03227` **an die Spitze**, dann C (0.8369 / bm25 0.5119 / fused 0.03226), dann B (nur Vektor, bm25 0, fused 0.01639). Der lexikalische Pfad holt den Unter-Boden-Treffer zurück und sortiert ihn korrekt vor das reine Vektor-Doc — 0.80-Andock-Riegel unberührt. Kern-Hebel damit auch im Browser bewiesen (headless 32/32 + Browser-Sichttest). Schritt-0-Baseline (KALIBRIER-BODEN / SCHWELLEN-ANALYSE / VERFAHREN-VERGLEICH) bleibt als Instrument in Panel 04 verfügbar. |
| Bau 04.G queryLocalJudged | 2026-07-01 | Bau-Sitzung Semantik-Matching Strang A2 (Brief 2026-07-01) | **Code in `src/modules/04_match.js` additiv** (keine bestehende Funktion verändert). STRANG A2: verankert den KI-Richter (`hybridMatch`) fest im **Antwort-Pfad** als EINE komponierte, opt-in Funktion — **ohne ein anderes Modul anzufassen** (Verdrahtung in Modul 15 bleibt eigener Folge-Schritt). Neue async-Funktion `queryLocalJudged(text, k, options?) → Promise<{judged, candidates, judgment}>`: (1) **Vorfilter** `queryLocal(text, k, {hybrid?})` (A1-Hybrid wird durchgereicht), (2) **Richter** (opt-in/BYOK) — nur wenn `options.apiKey` gesetzt, urteilt `hybridMatch` über die Finalisten und sortiert um (`passt` zuerst, dann Richter-Score, Tie-Break Cosinus), (3) **fail-soft** — kein Schlüssel / leerer Vorfilter / Richter nicht erreichbar → rohes Vorfilter-Ergebnis gilt, KEIN Throw. Der Richter beurteilt den Passage-**Text** — `queryLocalJudged` löst den Korpus **identisch zu queryLocal** auf (options.corpus | registrierter Provider) und baut die Text-Karte (anchorId bevorzugt, sonst label; Fallback label). Treffer tragen additiv `passt`/`judgeScore`/`begruendung`; `score` bleibt der Cosinus. Kandidaten-Cap `HYBRID_MAX_CANDIDATES` (20), Rest un-gerichtet hinten angehängt. **`_meta`** um `queryLocalJudgedNote`, Selbstcheck-Zeile nennt `queryLocalJudged`. **Headless-Smoke** `tests/smoke_bau04g_query_local_judged.mjs`: **28/28 grün** (Export-Anker, Opt-in-aus ohne fetch-Call, leerer Vorfilter, Richter-Umsortierung + Bezeugung, Fail-soft, Passage-Text erreicht Richter, Hybrid-Durchreichung). Regression 04c 43/43, 04d 68/68, 04e 29/29, 04f 32/32, Drift-Guard standalone 46/46 + bundle 21/21 grün. `node --check` grün. **Panel 04 Test 21** (Vorfilter vs. Mock-Richter). **TABU:** `PROVIDER_MIN_MATCH` (0.80) + Andock-Riegel (Modul 05) unberührt, kein Schlüssel im Code, kein PROTOCOL_VERSION-/DB_VERSION-Bump, kein Modul-Eingriff außer 04. Byte-Kopien `such-tool/` + `sbkim-bundle/` mitgezogen. **Browser-Sichttest (Test 21 + echter Richter-Schlüssel) wartet auf Klaus.** |
| Sichttest (Bau 04.G) | — | Klaus | **ausstehend** — Panel 04 **Test 21** „queryLocalJudged" (ohne Schlüssel = Vorfilter-Reihenfolge; mit Mock-Richter = umsortiert, passt zuerst). Headless 28/28 grün ersetzt den Browser-Sichttest nicht. |
| Bau 04.H Query-Expansion / Multi-Query | 2026-07-01 | Bau-Sitzung Semantik-Matching Strang A4 | **Code in `src/modules/04_match.js` additiv** (keine bestehende Funktion verändert). STRANG A4: aus EINER Frage mehrere Formulierungen bilden und die Trefferlisten verschmelzen — findet Ziele, die eine einzelne Formulierung verpasst. **Zwei neue Funktionen:** `expandQuerySimple(text, {synonyms?, maxVariants?})` (freie/offline Varianten-Erzeugung über eine optionale Synonym-Karte, Original zuerst, dedupe, Deckel `MULTI_MAX_VARIANTS=8`; ohne Karte → `[text]`, kein Netz/LLM) + `queryLocalMulti(queries, k, options?)` (sucht mit JEDER Variante via `queryLocal` — options inkl. `hybrid` durchgereicht — und verschmilzt die Rang-Listen via **RRF** `1/(RRF_K+rang)`; `score`=bester Cosinus, `matchedQueries`=#Varianten; fail-soft je Variante, `perQueryK`-Kandidatentiefe Default `max(k·3,10)`). **`_meta`** um `multiMaxVariants` + `queryLocalMultiNote`, Selbstcheck-Zeile nennt `queryLocalMulti/expandQuerySimple`. **Headless-Smoke** `tests/smoke_a4_query_expansion.mjs`: **16/16 grün** (expandQuerySimple Varianten/dedupe/Deckel/leer-Throw; queryLocalMulti Cross-Phrasing-Rettung, Varianten-Dedupe, Fail-soft, k-Cut, leer-Throws; queryLocal unverändert). Regression 04a/04c/04d/04e/04f/04g grün; Drift-Guards such-tool 49/49 + sbkim-bundle 21/21 byte-1:1 (pinnwand hat kein Modul 04). **Panel 04 „A4-NACHMESSUNG"**-Knopf (deterministischer Hash-Mock, lokal gesichert+restauriert). **TABU:** `PROVIDER_MIN_MATCH` (0.80) + Andock-Riegel (Modul 05) unberührt, kein Netz/LLM nötig (freier Pfad), kein PROTOCOL_VERSION-/DB_VERSION-Bump, kein Modul-Eingriff außer 04. **Browser-Sichttest (A4-NACHMESSUNG) + App-Integration (Modul 22: Synonym-Karte / opt-in LLM-Varianten-Generator) wartet auf Klaus.** |
