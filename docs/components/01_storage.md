...
#### Folge-Pflege 2026-05-21 — Race-Auflösung in openProbe + Close-Wait

**Was sich änderte:** Die Pflege „Versions-Bump-Race in `openProbe`"
vom 2026-05-21 hat drei additive Eingriffe in den Bau-01.Y- /
Pflege-`init()`-versions-fail-soft-Pfad eingebaut, ohne das Verhalten
von außen zu brechen. (1) Ein neuer modul-lokaler Helper
`closeConnectionAndWait(db)` wartet auf `db.onclose`-Feuer ODER auf
einen 50-ms-Timeout-Fallback (Chrome feuert `onclose` nicht
zuverlässig auf Android — der Timeout ist die Sicherheits-Klausel).
(2) Die beiden `probedDb.close()`-Stellen in `init()` (Fail-soft-Pfad
+ Initial-Pfad) sowie der `db.close()` vor dem Versions-Bump in
`ensureStore` nutzen jetzt `closeConnectionAndWait` async-await statt
synchroner Aufruf — der nachfolgende `indexedDB.open(name,
newVersion)` startet erst, wenn die Vorgänger-Verbindung im IDB-
Worker-Thread vollständig geschlossen ist. (3) `openProbe` installiert
`attachVersionChangeHandler` AUF der Probe-Verbindung; sie ist zwar
transient, muss aber im IDB-Worker-Thread sicher schließen können,
falls ein späterer Bump das `onversionchange`-Event auslöst.

**Klaus' Sichttest-Beweis:** Klaus' Sichttest am 2026-05-21 (DeX-
Chrome auf Galaxy Tab S6) hatte den Bug reproduzierbar gemacht — nach
Panel-01-Notfall-Reset (PR #131) + Hard-Reload + Panel-06-Setup warf
Modul 06 `ensureStore('sbkim_meta') Versions-Bump blockiert — ein
anderer Tab haelt die DB offen und ignoriert onversionchange.`
(Befund 2026-05-21 im Übergabeprotokoll § Diagnose 2). Nach der
Pflege MUSS dieselbe Sequenz ohne Throw durchgehen. Manifestation nur
in `tests/manual_check.html` bei wiederholtem Modul-Wechsel (mehrere
`init()`-Ketten pro Tab-Session) — **Endknoten-PWAs sind nicht
betroffen**, weil sie nur EINE `init()`-Kette pro Tab-Lebenszeit
haben (Karte 09 § Schritt 9 ruft `init()` einmal beim Andocken).
...
| Sichttest Bau-Pipeline-vollständig | 2026-05-20 | Klaus + Sichttest-Nachzug | **Klaus' Sichttest 2026-05-20 grundbelegt** (DeX-Chrome auf Galaxy Tab S6, Termux-`python3 -m http.server 8000`-Setup): Panel 01 Storage-init zeigt `db_version: 7` mit aktuell 11 Stores inkl. **`sbkim_siblings_main`** (Bau-05.Y-Schreib-Pfad live — Modul 05's `ensureSlotStores` im `init()` legt diesen Store an); `sbkim_test_failsoft_dummy` + `sbkim_test_foo` (akkumulierte Test-Stores aus früheren Sichttests, Pflege-Modul-01-versions-fail-soft toleriert sie). Knöpfe 1-3 (init / round-trip / UnknownStoreError) grün; Knopf 4 (Selbstcheck) zeigt erwartete `init/getStore/get/put/del/all/clear`-Funktions-Liste; Knopf 5 (Persist-Status) liefert `storagePersisted: false` (Termux-`localhost`-Setup ohne PWA-Origin — erwartete Browser-Restriktion, nicht Modul-Bug); Knopf 6 ensureStore happy-path grün (`db_version_vor: 7`, `db_version_nach: 7` — Idempotenz greift, weil `sbkim_test_foo` aus früheren Sichttests bereits existiert); Knopf 7 ensureStore zweimal grün (alle drei `db_version`-Werte bleiben 7, `idempotent: true`). Klaus' DeX-Chrome-Setup ist mit dem akkumulierten Test-State auf v=7 stabil ohne `VersionError`-Befund. |
| Pflege „Versions-Bump-Race in openProbe" | 2026-05-22 | Pflege Modul 01 Versions-Bump-Race | Folge-Pflege auf Klaus' Sichttest 2026-05-21 (Sichttest-Folge zur Bau-Sage-Page-Refactor-Sitzung, PRs #127–#134 gemerged) und Diagnose-2-Befund im Übergabeprotokoll: `ensureStore('sbkim_meta') Versions-Bump blockiert` reproduzierbar auf frischer DB nach Panel-01-Notfall-Reset (PR #131) + Hard-Reload + Panel-06-Setup. **Tafel-Evolutions-konform** (CLAUDE.md § Heilige Tafeln § Tafel-Evolutions-Klausel): additive Race-Auflösung im Bau-01.Y- / Pflege-`init()`-versions-fail-soft-Pfad, **KEIN INTERFACES-Bietet-/Storage-/Fehler-Block-Eingriff**, **KEIN `ensureStore`-Verhalten-Bruch von außen**, **KEIN `DB_VERSION`-Bump** (`DB_VERSION = 4` bleibt). **Code in `src/modules/01_storage.js` additiv** (kein Refactoring der bestehenden 8 Funktionen): neuer modul-lokaler Helper `closeConnectionAndWait(db)` zwischen `attachVersionChangeHandler` und `requestStoragePersist` (wartet auf `db.onclose`-Feuer ODER 50-ms-Timeout-Fallback — Chrome feuert `onclose` nicht zuverlässig auf Android, der Timeout ist die Sicherheits-Klausel); `openProbe(name)` ruft `attachVersionChangeHandler(req.result)` vor `resolve` (Probe-Verbindung trägt jetzt den fail-soft-`onversionchange`-Handler — sie ist zwar transient, muss aber im IDB-Worker-Thread sicher schließen können bei späterem Bump); `init()` beide `probedDb.close()`-Stellen (Fail-soft-Pfad bei `existing >= DB_VERSION` + Initial-Pfad) ersetzt durch `closeConnectionAndWait(probedDb).then(...)` — der nachfolgende `openExact`/`indexedDB.open` startet erst, wenn die Probe-Verbindung im DB-Worker vollständig geschlossen ist; `ensureStore` `db.close()` vor dem Versions-Bump (Zeile 770) ersetzt durch `closeConnectionAndWait(db).then(...)` mit `indexedDB.open(dbNameInUse, newVersion)` im `then`-Block (currentDb/dbPromise-Invalidierung BLEIBT vor dem await, damit nachfolgende `init()`-Aufrufe sofort auf der neuen Verbindung landen). **Modul-Kopfkommentar** um Pflege-Block am Anfang erweitert (Befund, Ursache, drei Eingriffe). **Headless-Smoke-Test** `tests/smoke_pflege_01_versions_bump_race.mjs` (Node 22 + fake-indexeddb): vier Proben, 6 Sub-Proben, 6/6 grün — zwei aufeinander folgende `ensureStore`-Bumps ohne Block (Probe 1) / `closeConnectionAndWait`-Timeout-Fallback greift (Probe 2 zeigt ~51 ms pro Bump — der 50-ms-Timeout dominiert, weil fake-indexeddb kein `onclose` für normalen `close()` feuert; das spiegelt das Android-Chrome-Verhalten) / drei `init()`→`ensureStore`-Zyklen mit Modul-Re-Load ohne `EnsureStoreError` (Probe 3 — Klaus' Sichttest-Sequenz) / Regression: bestehende init-Garantien greifen weiter (Probe 4). **Regression** alle anderen Smoke-Tests grün ohne Anpassung: Pflege-01-Smoke 8/8 + Bau-02.Y 33/33 + Bau-04.A 19/19 + Bau-05.Y 25/25 + Bau-06.Y 25/25 + Bau-07.Y 30/30 + Bau-08.Y 26/26 = 166 Proben grün. **KEINE Modul-02/05/06/07/08-Änderung**, **KEINE `tests/manual_check.html`-Änderung** (Sichttest-Trigger ist der vorhandene PR-#131-Notfall-Reset-Knopf + Hard-Reload + Panel-06-Setup-Knopf), **KEINE Sage-Page-Änderung**, **KEINE CLAUDE.md-/Karte-09-/`status.json`-Änderung**. **`PROTOCOL_VERSION` bleibt `"0.1"`, `DB_VERSION` bleibt `4`, `BACKUP_FORMAT_VERSION` bleibt `2`**. **`status.json` unverändert** (Modul 01 bleibt `score:"fertig"`; `update_puls_pie.py` NICHT aufgerufen). `node --check src/modules/01_storage.js` grün. **Race-Auflösung grün im Sichttest am Galaxy Tab S6 / DeX-Chrome** (siehe nachfolgende Bauzustand-Zeile). |
| Sichttest Race-Auflösung (Pflege „Versions-Bump-Race") | 2026-05-22 | Klaus + Pflege Modul 01 Versions-Bump-Race | **grün geprüft 2026-05-22 (Klaus, DeX-Chrome auf Galaxy Tab S6, Termux-`python3 -m http.server 8000`-Setup):** 11-Knopf-Sequenz aus dem Brief vollständig durchgespielt, **alle 11 Klicks grün** — Pfad genau der, der vor der Pflege reproduzierbar mit `ensureStore Versions-Bump blockiert` brach. (1) Panel-01-„Notfall-Reset" → DB `sbkim` gelöscht; (2) Strg+Shift+R Hard-Reload → Tab frisch; (3) **Panel-06-„Setup: Identität + 2 Pseudo-Geschwister" ohne Throw** (zentraler Race-Auflösungs-Beweis — `eigene_node_id: UnvQCNPhthiUCfb3CX3nzi7yGHBqvvzRvC_fA0Va858`, beide Pseudo-Geschwister geschrieben, Setup-OK-Chip); (4) Panel-06-Test-1 „Lokaler Pull-Round-Trip" → `outcome:"shared"`, `anchor_count:1`, Signatur valide, Inbox hat Eintrag; (5) Panel-06-Test-9 „HETERO_MAX_ANCHORS-Begrenzung" → 6 Outbox-Einträge → `anchor_count:5`, „Nachtisch" zuerst, „Hefeteig" aussortiert; (6) Panel-06-Test-10 „listHeterokaryosis" → 4 Inbox-Einträge, Form korrekt, `anchors`-Inhalte UI-seitig weggelassen; (7) Panel-06-Test-11 „forgetHeterokaryosis idempotent" → `vor_forget:1 → nach_forget:0`, zweiter Aufruf wirft nicht; (8) Panel-07-Test-4 „TTL-Cleanup" → `entfernt_anzahl:1`, altOld weg, altYoung bleibt; (9) Panel-07-Test-5 „listLegacy" → 3 Demo-Einträge ohne `signature`-Feld; (10) Panel-07-Test-6 „Self-Apoptose IRREVERSIBEL" → `outcome:"completed"`, `stores_alle_leer:true`, `getNodeId_wirft_NoIdentityError:true` (Pseudo-Endpoints in `recipients_failed` erwartet); (11) Panel-00-Test-5 „TTL-Sweep" → `entfernt_anzahl:2`, beide alten Siblings entfernt, Snapshot leer. 50-ms-Timeout-Fallback in `closeConnectionAndWait` greift auf Android-Chrome erwartungs-gemäß — `onclose` feuert dort weniger zuverlässig als auf Desktop-Chrome. **Die 84 Bridge-Stellen aus PR #130-Test-Bridge-Pflege jetzt nicht nur statisch verifiziert, sondern live bestätigt** in den Panels 06/07/00. |
