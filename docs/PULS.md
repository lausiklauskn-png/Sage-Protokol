...
### ⚠ UND DANN WAR ES SCHLIMMER — der erste Bau hat die Ursache VERGRÖSSERT

Klaus nach dem Sichttest: *„jetzt noch schlimmer als vorher … es sieht immer
noch so aus, als wenn alles zu weit nach links gerückt ist. Sowohl die festen
Container als auch die beweglichen."*

**Er hatte recht, und der Fehler war meiner.** Gemessen bei drei
Fensterbreiten, `documentElement.scrollWidth` gegen `clientWidth`:

| | Seite breiter als das Fenster |
|---|---|
| vor dem ersten Bau | **7,3 %** |
| nach dem ersten Bau | **10,6 %** |
| nach der Berichtigung | **0,0 %** |

**Der Mechanismus, und er ist der eigentliche Befund dieser Sitzung.** Eine
Kamerafahrt vergrößert ihre Schicht und schiebt sie; was dabei über die
Sektion hinausragt, zählt als **Scroll-Überlauf des Dokuments**. Ein Browser,
der eine zu breite Seite ins Fenster einpasst, zeichnet dann **alles kleiner
und linksbündig** — und rechts bleibt ein Streifen. Genau das hatte Klaus
fotografiert.

**Und die Zahl passt:** seine Inhaltskante in Sektion 1 lag bei **90,2 %** der
Bildbreite, und `100 / 110,6 = 90,4 %`.

⚠ **ICH HABE DEN ÜBERHANG VON 0,22 % AUF 3,01 % GEHOBEN — und damit den
Überlauf von 7,3 % auf 10,6 %.** Die Abhilfe gegen den Haarstrich war die
Ursache des Streifens. Beide Male ging es um dieselbe Zahl, nur in
verschiedene Richtungen gelesen.

⚠ **UND KEINE PROBE KONNTE ES SEHEN.** Headless Chromium passt eine zu breite
Seite **nie** ins Fenster ein — es zeichnet sie einfach und clippt. Der
Streifen ist dort unsichtbar, mit und ohne Fehler. Gefunden hat es Klaus'
Sichttest und danach eine Zahl, nach der vorher niemand gefragt hatte.
*Der Sichttest ist nicht ersetzbar, und diesmal hat er es bewiesen.*

### Die Berichtigung — Klaus' eigener Vorschlag war der bessere Bau

Er schrieb: *„Das ist ja nicht schlimm, dass die Seite breiter ist als das
Fenster, wenn das der Hintergrundeffekt ist … er müsste eher sogar noch
breiter sein."* Und: *„Kannst du es nicht auch so bauen, dass sie sich
automatisch an die Bildschirmbreite anpassen … dass sie sich dann breiter
ziehen."*

**Beides stimmt, und beides ist jetzt so gebaut** — nur an zwei verschiedenen
Stellen, und der Unterschied ist die ganze Sache:

| | |
|---|---|
| die **Schicht** darf und soll breiter sein als das Fenster | `inset: -8%` — auf jeder Seite 8 % Spielraum, in Prozent, passt sich also jeder Bildschirmbreite von selbst an |
| die **Seite** darf es nicht | `overflow: clip` an den drei Foto-Sektionen schneidet ab, was übersteht |

⚠ **DER SPIELRAUM GEHÖRT IN DEN KASTEN, NICHT IN DIE BEWEGUNG.** Ein größeres
`scale` schafft denselben Spielraum, zoomt dabei aber das Bild — es ändert die
Bildwirkung, um ein Randproblem zu lösen. Und es hält nur, solange niemand an
den Keyframes dreht. Deshalb stehen die **Kamerafahrten wieder auf ihren
ursprünglichen Werten** (1.08 → 1.04 usw.); die Bewegung ist wieder genau die,
die Klaus kannte.

⚠ **NICHT AN DIE TÜR-SEKTION.** Sie trägt eine `position: sticky`-Bühne, und
`overflow` an einem Vorfahren macht ihn zum Scroll-Bereich — dann pinnt sie
dort, wo niemand es erwartet. Dieselbe Falle, die am `<body>` schon einmal
aufgeschrieben ist. Eigener Wächter, eigener Gegenprobe-Fall.

### Der Deckungs-Wächter misst jetzt die MATRIX, nicht die Keyframes

Die erste Fassung rechnete den Überhang aus den Keyframe-Werten. Das war
richtig, solange der Spielraum dort stand — mit `inset: -8%` weiß eine solche
Rechnung nichts von dem Spielraum und hätte 0,22 % gemeldet, während wirklich
8,8 % gedeckt sind.

Gemessen wird jetzt der **gezeichnete** Kasten: `getComputedStyle` liefert an
der laufenden Animation die Matrix, dazu kommen Größe und Lage der Schicht,
und daraus die vier Kanten. Fünf Punkte je Fahrt.

| Fahrt | Deckung an der knappsten Stelle |
|---|---|
| Sektion 1 | **8,83 %** |
| Sektion 5 | **8,62 %** |
| Sektion 6 | **9,59 %** |

### ⚠ Und ein Gegenprobe-Fall war nach der Berichtigung selbst überholt

„Die Fahrten haben gar keinen Überhang mehr" setzte **eine** Fahrt auf
`scale(1.0)`. Solange der Spielraum in der Bewegung stand, nahm das der
Schicht ihren Überhang; seit er am Kasten steht, nicht mehr. Der Fall
rutschte durch — **gemeldet vom Lauf, nicht vom Nachdenken.** Er nimmt jetzt
Spielraum und Bewegung auf einmal.
...
eingefügt … Und die Umlaute hast du nicht beachtet, die in ganz normaler Art und
Weise darstellen, wie zum Beispiel Lösungsschlüssel."* Eine Stelle hat er beim
Namen genannt: *„Emotionserkennung am Arbeitsplatz und in Bildungseinrichtungen"*.

Die Regel steht im Skill `menschlich-schreiben` und ist nicht neu: höchstens ein
Gedankenstrich auf drei bis vier Absätze. Ich hatte sie nicht angewandt.

**Gemessen am sichtbaren Text** (`innerText` im Browser, samt Knopf-Beschriftungen,
`title` und Platzhaltern), nicht am Quelltext:

| | vorher | nachher |
|---|---|---|
| Gedankenstriche | **11** | **1** |
| Umschrift statt Umlaut | 2 | **0** |

Der eine verbliebene Strich steht in den Titeln der amtlichen Quellen
(*„AI Literacy - Questions & Answers"*). Ein zitierter Titel gehört wörtlich;
ihn zu glätten machte die Quellenangabe falsch. Benannte Grenze.

⚠ **UND MEINE STRICH-SUCHE HAT DIE STELLE ZUERST NICHT GEFUNDEN, DIE KLAUS
NANNTE.** `grep -E ' [-–—] '` greift bei einer Zeichenklasse mit mehrbyte-Zeichen
daneben; gemeldet wurden 4 statt 11. **Eine Suche, die zu wenig findet, sieht
genauso aus wie ein sauberer Befund.** Gezählt wird seitdem jede Strichsorte
einzeln.
...
(`Ueberschrift`, `laesst`, `fuer`). Mit nachgezogen; in der ganzen Datei steht
jetzt keine Umschrift und kein Gedankenstrich mehr.

### ⚠ Die Zahl, die sich selbst widersprach

Beim Durchsehen fiel etwas auf, wonach niemand gesucht hatte: die Überschrift
sagte **„Wissenstest - 15 Fragen"**. Im Formular stehen **16** `<fieldset>`, in
der Auswertungs-Liste **16** Einträge, und die Bescheinigung rechnet mit
`____ / 16`.

⚠ **DER KOMMENTAR DANEBEN VERSPRACH DIE KOPPLUNG, DIE ES NICHT GAB:** *„Wer eine
Frage ergänzt, ändert nur die Liste oben; Text, Zeugnis und Schlüssel ziehen
nach."* Zeugnis und Schlüssel zogen nach, die Überschrift stand fest im Markup.
Genau daraus ist die 15 geworden. Sie zieht jetzt mit (`<span id="quizCount">`),
gesetzt aus derselben `TOTAL`; der feste Wert bleibt als Rückfall ohne
JavaScript stehen, wie bei `certScore`.

⚠ **DER WÄCHTER MISST DIE ÜBEREINSTIMMUNG, NICHT DIE ZAHL.** Eine festgenagelte
16 wäre bei der siebzehnten Frage genauso falsch wie die 15 es war. Gezählt
werden die `<fieldset>`, dagegen gehalten wird die Überschrift.

### Was davor lag: die Bescheinigung war nie kaputt

Klaus hatte zuerst gemeldet, die Urkunde fehle im Ausdruck. **Gemessen an
derselben Datei:** erster Knopf 7 Seiten ohne Bescheinigung, dritter Knopf
(*„Schulung + Bescheinigung"*) 8 Seiten mit, eigener Knopf 1 Seite. Jeder Knopf
tat, was draufsteht. Klaus hat das selbst aufgelöst (*„Schulung ausdrucken heißt
Schulung ausdrucken"*), und die angefangene Umstellung der Knopfzeile wurde
**zurückgenommen**: ein veröffentlichtes Dokument wird nicht ohne Auftrag
geändert.
...
| `smoke_cache_version` | 12/12, Bump v121 → v122 erkannt |
| Gegenprobe, 4 neue Fälle | **4 gefangen · 0 durchgerutscht · 0 tote Anker** |

Jeder der vier Fälle ist **von Hand nachgestellt und die rote Zeile gelesen**;
jede trägt den Namen ihrer eigenen Zusicherung neben dem erwarteten Pin-Ausfall.

⚠ **Seitenumbruch gegengemessen**, denn Textänderungen verschieben Zeilen. Bei
**14 mm** bleiben es **7 Seiten und jede Seitengrenze ist identisch**; bei 22 mm
bleiben es 8 Seiten, dort rückt ein Aufzählungspunkt von Seite 5 auf Seite 4.

Stand der Unterlage: `md5 d5f140110a1ea4404a59b636e6fb92ed`, 233 Zeilen. Cache
Toolpoint **v62**, family-project **v122** (dort trugen 5 Dateien noch `?v=121`
fest verdrahtet; jede Adresse trägt jetzt dieselbe Zahl).

⚠ **PULS ausgelagert, nicht gekürzt:** der Eintrag *„Zwanzig von zwanzig"* vom
2026-09-16 steht jetzt vollständig unter
[`docs/sessions/archiv/2026-09-16_puls-eintrag-zwanzig-von-zwanzig.md`](sessions/archiv/2026-09-16_puls-eintrag-zwanzig-von-zwanzig.md),
im PULS ein Stummel. 2980 auf 2860 Zeilen.
...
> Dieser Eintrag stand bis zum 2026-09-18 hier in voller Länge (74 Zeilen).
> Die Datei stand bei 3.033 von 3.000; **ausgelagert, nicht gekürzt** — der
> Wortlaut steht vollständig in [`sessions/archiv/2026-09-16_puls-eintrag-einstellungen-sprache.md`](sessions/archiv/2026-09-16_puls-eintrag-einstellungen-sprache.md).

| Sitzung | Wortlaut |
|---|---|
| 2026-09-16 — 🌐 Der Einstellungs-Bildschirm sprach stellenweise nur Deutsch | [→ Archiv](sessions/archiv/2026-09-16_puls-eintrag-einstellungen-sprache.md) |
...
**Modul 15 in allen zwanzig Trägern**, und Klaus' eigentliche Frage davor: *„warum
machen wir eine Sitzung nach der anderen, ohne dass ein Ergebnis zu sehen ist?"* —
beantwortet, denn es gab für ihn buchstäblich nichts zu sehen: die Übersetzung
zeigt sich nur auf einer Seite mit `lang="en"`. Dazu die vier Wortkarten, die
beinahe verschwunden wären. Volltext:
[`docs/sessions/archiv/2026-09-16_puls-eintrag-zwanzig-von-zwanzig.md`](sessions/archiv/2026-09-16_puls-eintrag-zwanzig-von-zwanzig.md).

---

## Eine Sitzung vom 2026-09-16 (Rollout mit Filter) — ausgelagert am 2026-09-16

**Modul 15 in acht von zwanzig Trägern**, gefiltert nach Generation — die übrigen
zwölf hingen weiter zurück und brauchten einen eigenen Gang. Daraus kam der
Verteiler-Schalter `--nur-generation`, und der Lauf meldete veraltete
Arbeitsbäume, statt sie still zu überschreiben. Volltext:
[`docs/sessions/archiv/2026-09-16_puls-eintrag-rollout-acht.md`](sessions/archiv/2026-09-16_puls-eintrag-rollout-acht.md).

---

## Eine Sitzung vom 2026-09-16 (Zweisprachigkeit) — ausgelagert am 2026-09-16

**Privat-Brain, Mycel-Fenster zweisprachig** (98 Einträge, schlüssellos — der deutsche
Satz IST der Schlüssel). Der Fund, auf den es ankam: das Panel wird beim **Laden**
gebaut, nicht beim Öffnen — mit `<html lang="en">` danach stand es vollständig auf
Deutsch, obwohl Tabelle und `T()` tadellos waren. Ein Wächter auf die Tabelle hätte das
nie gesehen; gefunden hat es die Browser-Hälfte der Probe. Volltext:
[`docs/sessions/archiv/2026-09-16_puls-eintrag-zweisprachig.md`](sessions/archiv/2026-09-16_puls-eintrag-zweisprachig.md).
...
## Zwei Sitzungen vom 2026-09-16 — ausgelagert am 2026-09-16

Die **eine Kennung, eine Spore** (das Siegel schrieb `embeddingVersion` nicht,
und die Zahl, die kein Vertrag war) und die **Ordner-Zählung samt zu engem
Bump** (ein Worker, der gleich-ursprüngliche GETs cache-first beantwortet,
legt die Datei selbst ab — gemessen an drei Repos). Beide vollständig und
wortgleich in
[`sessions/archiv/2026-09-16_puls-auslagerung-16-teil1.md`](sessions/archiv/2026-09-16_puls-auslagerung-16-teil1.md).
...
Die Herauslösung des Andock-Wizards (`16b_andock_wizard.js`, ein Wizard statt
zwölf), die Sabotage, die in einen Commit ging, und der tote Anker, der als
blinder Wächter gelesen wurde. Vollständig und wortgleich in
[`sessions/archiv/2026-09_puls-auslagerung-14.md`](sessions/archiv/2026-09_puls-auslagerung-14.md).
...
> **↓ Ausgelagert.** Der Eintrag „SIEGEL UND LAMPEN SPRECHEN ENGLISCH — 19
> APPS" steht **wortwörtlich** in
> [`sessions/archiv/2026-09-15_puls-auslagerung.md`](sessions/archiv/2026-09-15_puls-auslagerung.md).
> Die Datei stand bei 3.013 von 3.000 Zeilen; **ausgelagert, nicht gekürzt.**
...
> **↓ Ausgelagert.** Der Eintrag „MODUL 23 UI IN 18 APPS — UND ZWEI LÜCKEN IN
> DER GESCHENKBOX" steht **wortwörtlich** in
> [`docs/sessions/archiv/2026-09_puls-auslagerung-12.md`](sessions/archiv/2026-09_puls-auslagerung-12.md)
> (128 Zeilen). Nichts gekürzt — auslagern statt kürzen.
...
> **↓ Ausgelagert am 2026-09-14 (spät).** Der Eintrag „DIE FRIST, DER SPRUNG
> UND DIE FÜNF UNTER 0,80" steht **wortwörtlich** in
> [`docs/sessions/archiv/2026-09_puls-auslagerung-8.md`](sessions/archiv/2026-09_puls-auslagerung-8.md)
> (160 Zeilen). Nichts gekürzt — die Schutz-Klausel oben verlangt auslagern
> statt kürzen, und die Git-Historie trägt es ohnehin.

## Zwei Sitzungen vom 2026-09-10/11 — ausgelagert am 2026-09-14

„KEIN LINK FÜHRT MEHR NIRGENDWOHIN“ (2026-09-11) und „DOPPELTE EINSTIEGE WEG,
EINGEBETTETE KARTE NACHGEZOGEN“ (2026-09-10, Nacht) stehen in voller Länge in
[`sessions/archiv/2026-09-14_puls-auslagerung-4.md`](sessions/archiv/2026-09-14_puls-auslagerung-4.md).
Nichts gekürzt — nur umgezogen, damit die 3000-Zeilen-Grenze hält.
...
> stand nach dem Eintrag von heute bei **3.051 von 3.000**; **ausgelagert, nicht
> gekürzt** — der Wortlaut steht vollständig in
> [`sessions/archiv/2026-09-14_puls-auslagerung.md`](sessions/archiv/2026-09-14_puls-auslagerung.md).
...
## Drei Sitzungen vom 2026-09-08 — ausgelagert

> Diese drei Einträge standen bis zum 2026-09-10 hier in voller Länge (294 Zeilen).
> Die Datei stand bei 2.991 von 3.000; **ausgelagert, nicht gekürzt** — der Wortlaut
> steht vollständig in [`sessions/archiv/2026-09-08_puls-eintraege-drei-sitzungen.md`](sessions/archiv/2026-09-08_puls-eintraege-drei-sitzungen.md).

| Sitzung | Wortlaut | Übergabeprotokoll |
|---|---|---|
| 2026-09-08 (Haupt-Sitzung, spät) — ✅ DER GETEILTE VORRAT IST BEI NULL | [→ Archiv](sessions/archiv/2026-09-08_puls-eintraege-drei-sitzungen.md) | [→ Protokoll](sessions/archiv/2026-09-08_vorrat-letzte-stelle.md) |
| 2026-09-08 (Bau-Sitzung, abends) — 🧹 DER GETEILTE VORRAT IST REPARIERT | [→ Archiv](sessions/archiv/2026-09-08_puls-eintraege-drei-sitzungen.md) | [→ Protokoll](sessions/archiv/2026-09-08_geteilter-vorrat-reparatur.md) |
| 2026-09-08 (Mess-Sitzung) — 🧹 DER GETEILTE VORRAT, die Apps löschen einander die Offline-Vorräte | [→ Archiv](sessions/archiv/2026-09-08_puls-eintraege-drei-sitzungen.md) | [→ Protokoll](sessions/archiv/2026-09-08_geteilter-vorrat.md) |
...
## Vier Sitzungen vom 2026-08-24 — ausgelagert

> Diese vier Einträge standen bis zum 2026-09-03 hier in voller Länge (327 Zeilen).
> Die Datei stand bei 2.916 von 3.000; **ausgelagert, nicht gekürzt** — der Wortlaut
> steht vollständig in [`sessions/archiv/2026-08-24_puls-eintraege-vier-sitzungen.md`](sessions/archiv/2026-08-24_puls-eintraege-vier-sitzungen.md).

| Sitzung | Wortlaut |
|---|---|
| 2026-08-24 (Bau, 3.) — ⏱ Die Arbeitstage einzeln, für das Finanzamt | [→ Archiv](sessions/archiv/2026-08-24_puls-eintraege-vier-sitzungen.md) |
| 2026-08-24 (Bau, 2.) — 🟡 Die gelbe Runde, und zwei Funde daneben | [→ Archiv](sessions/archiv/2026-08-24_puls-eintraege-vier-sitzungen.md) |
| 2026-08-24 (Bau) — 📜 Die Historie ausgelesen, Gedankenstriche raus | [→ Archiv](sessions/archiv/2026-08-24_puls-eintraege-vier-sitzungen.md) |
| 2026-08-24 (Bau) — ✎ Markieren in der Antragsmappe (Kürzen) | [→ Archiv](sessions/archiv/2026-08-24_puls-eintraege-vier-sitzungen.md) |


## Fünf Sitzungen vom 2026-08-23 — ausgelagert

> Diese fünf Einträge standen bis zum 2026-09-02 hier in voller Länge (210 Zeilen).
> Die Datei stand an ihrer Grenze; **ausgelagert, nicht gekürzt** — der Wortlaut steht
> vollständig in [`sessions/archiv/2026-08-23_puls-eintraege-fuenf-sitzungen.md`](sessions/archiv/2026-08-23_puls-eintraege-fuenf-sitzungen.md).

| Sitzung | Wortlaut |
|---|---|
| 2026-08-23 (Bau) — 📦 Antragsmappe: eine Datei, zwei Abteilungen | [→ Archiv](sessions/archiv/2026-08-23_puls-eintraege-fuenf-sitzungen.md) |
| 2026-08-23 (Bau) — 📄 Paper A geschrieben | [→ Archiv](sessions/archiv/2026-08-23_puls-eintraege-fuenf-sitzungen.md) |
| 2026-08-23 (Bau) — 🇩🇪 Hamburg nachgezogen, Paper-Plan angelegt | [→ Archiv](sessions/archiv/2026-08-23_puls-eintraege-fuenf-sitzungen.md) |
| 2026-08-23 (Bau) — 🔓 Der Forschungskorpus steht | [→ Archiv](sessions/archiv/2026-08-23_puls-eintraege-fuenf-sitzungen.md) |
| 2026-08-23 (Recherche) — eine Recherche-Sitzung | [→ Archiv](sessions/archiv/2026-08-23_puls-eintraege-fuenf-sitzungen.md) |
