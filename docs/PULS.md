...
offene Bau-Sitzung aus dem Spec-Sitzung-08-Block (2026-05-15);
schließt die Reihe Spec 08 → Pflege Bau 06.1 → Bau 08 sauber ab.

**Getan:**

- **`src/modules/08_ui_demo.js` neu angelegt** — IIFE mit
  `window.SbkimUiDemo`, fünf öffentliche Funktionen (`init/list
  Outbox/addOutboxAnchor/removeOutboxAnchor/setSiblingHeteroOptIn`),
  sechs benannte Error-Klassen im Factory-Stil analog Modul 00
  (`UiDemoDependenciesError`, `InvalidAnchorLabelError`,
  `InvalidAnchorVectorError`, `OutboxFullError`,
  `UnknownSiblingError`, `InvalidOptInArgError` — alle auf
  `SbkimUiDemo.<Error>` exportiert für `err.name`-Vergleiche /
  `instanceof Error`-Checks), drei Test-Brücken (`_clearOutbox`,
  `_addPseudoSibling` ohne `heterokaryosisOptIn`-Flag,
  `_clearPseudoSiblings`), synchroner Selbstcheck `MODUL 08 UI-DEMO
  bereit, Funktionen: init/listOutbox/addOutboxAnchor/removeOutbox
  Anchor/setSiblingHeteroOptIn` beim Skript-Laden (vor jeglichem
  Aufruf, nicht in `init()`). Modul 08 ist **nicht protokoll-aktiv**:
  kein `fetch`, kein Service-Worker-Hook, kein `SbkimEmbedding`-
  Aufruf, keine Signatur, kein Heterokaryose-Pull. Vektor-Erzeugung
  ist Aufrufer-Pflicht (typisch `SbkimEmbedding.embedPassage` im
  Endknoten-UI-Code, bevor `addOutboxAnchor` gerufen wird).

- **Bau-Pflicht-Entscheidung 1:** Reihenfolge der Checks in
  `addOutboxAnchor` — (1) `label`-Typ + Länge sync →
  `InvalidAnchorLabelError`, (2) `vector`-Form sync (Array, Länge
  `EMBEDDING_DIM = 384`, alle Werte `Number.isFinite`) →
  `InvalidAnchorVectorError`, (3) async-Existenz-Check
  (`SbkimStorage.get`) + Voll-Check (`SbkimStorage.all` → `length
  >= HETERO_OUTBOX_MAX_ENTRIES`) → `OutboxFullError` **nur wenn das
  Label NEU ist**. Überschreiben eines bekannten Labels bleibt
  trotz Voll-Status erlaubt (kein Verdrängen, Anzahl unverändert);
  `addedAt` wird auf `nowIso()` aktualisiert.

- **Bau-Pflicht-Entscheidung 2:** `init()` idempotent über internen
  `ready`-Flag; `options`-Werte werden bei jedem Aufruf aktualisiert
  (sodass spätere Aufrufer die Konfiguration anpassen können), aber
  `SbkimStorage.init()` läuft nur einmal effektiv und der Selbstcheck
  steht synchron im IIFE-Body — er wird also genau einmal beim
  Skript-Laden geloggt, nicht pro `init()`.

- **Bau-Pflicht-Entscheidung 3:** `setSiblingHeteroOptIn` strikt
  boolean — `optIn !== true && optIn !== false` wirft
  `InvalidOptInArgError` (kein truthy/falsy-Cast; `1`, `null`,
  `undefined`, `"true"` werden alle abgelehnt). Co-Schreiber-
  Disziplin via `Object.assign({}, sibling, {heterokaryosisOptIn:
  optIn})` — alle anderen Felder (`nodeId`, `domain`, `endpoint`,
  `pubKey`, `since`) bleiben unverändert. Wenn `peerNodeId` nicht
  in `sbkim_siblings` → `UnknownSiblingError` (Modul 08 legt
  KEINEN neuen Sibling-Eintrag an — Modul 05 bleibt Haupt-
  Schreiber).

- **Bau-Pflicht-Entscheidung 4:** `_addPseudoSibling`-Test-Brücke
  schreibt **KEIN** `heterokaryosisOptIn`-Feld (anders als Modul 06's
  `_addPseudoSibling`, das das Flag mit-akzeptiert). Begründung:
  Panel-08-Test 6 muss den **Co-Schreiber-Pfad** prüfen — d.h.
  Modul 08's `setSiblingHeteroOptIn` muss das Feld auf einen
  bestehenden Sibling-Eintrag SETZEN. Würde die Test-Brücke das Feld
  schon mitschreiben, wäre der Co-Schreiber-Pfad nicht getestet.

- **Bau-Pflicht-Entscheidung 5:** Panel 08 in
  `tests/manual_check.html` ersetzt den vorherigen Werkstatt-Stub
  (Status-Chip `idle "in dieser Datei selbst"` mit Text „Diese Seite
  ist Modul 08.") durch ein vollwertiges Panel mit Status-Chip
  `ok "Code-Stub"` und Hinweis-Block (Storage-only, Embedding-frei,
  Self-Apoptose-Knopf bewusst nicht hier). Das löst die alte
  Werkstatt-Doppelbedeutung sauber auf: nach Spec-Sitzung 08 §
  Modul-08-Rollenwahl ist `tests/manual_check.html` ausdrücklich
  KEIN Modul-08-Code (Variante b verbindlich) — Panel 08 ist jetzt
  eine **Test-Oberfläche für Modul 08**, kein Eigen-Modul. Acht
  Knöpfe: Setup (`_clearOutbox` + `_clearPseudoSiblings`), Test 1
  (Outbox add + list, drei Anker, absteigend nach `addedAt`,
  Vektoren NICHT in der UI-Antwort), Test 2 (Remove idempotent),
  Test 3 (Überschreiben — `addedAt` frisch, Anzahl unverändert),
  Test 4 (`OutboxFullError` bei NEUEM Label, Überschreiben +
  Free-Up-Pfad), Test 5 (sechs Validierungs-Würfe synchron, Outbox
  bleibt leer), Test 6 (`setSiblingHeteroOptIn` true/false +
  `UnknownSiblingError` + `InvalidOptInArgError` für `1` und
  `"true"`), Selbstcheck-Hinweis. `<script src=
  "../src/modules/08_ui_demo.js"></script>` zwischen Modul 06 und
  Modul 00 eingehängt (Reihenfolge im File: 01 → 03 → 04 → 02 → 05
  → 07 → 06 → 08 → 00 — Modul 08 hat nur eine Pflicht-Abhängigkeit
  auf `SbkimStorage` und ist unabhängig von Modul 06).

- **Karte 08 § Bauzustand-Zeile „Code geschrieben | 2026-05-15 | Bau 08"**
  ausführlich ergänzt (analog Bau-Sitzung 06 in Karte 06):
  Bau-Pflicht-Entscheidungen 1-5 mit Begründung, Verbote-Liste,
  Validierungs-Ergebnisse, Pie-Bewegung.

- **INTERFACES.md §6 Änderungsprotokoll-Zeile** „Bau-Sitzung 08"
  am Ende ergänzt (nach Pflege Bau 06.1) — §1 Modul 08 unverändert
  (Vertrag steht seit Spec-Sitzung 08).

- **`status.json` Modul 08** von `score:"spec"` / `siegel:"Spec
  fertig"` auf `score:"stub"` / `siegel:"Code-Stub"` hochgestuft;
  `kurz` um Bauzustand-Hinweis erweitert (sechs Error-Klassen,
  Storage-only-Charakter, v=3-Store-Bezug auf Pflege Bau 06.1);
...
- **Pie regeneriert** via `python3 scripts/update_puls_pie.py`:
  ...
  **Code-Stub 8 → 9**, Fertig 0 → 0 (14 Module gesamt unverändert).
  Die `Spec fertig`-Zeile zählt jetzt nur noch Modul 09 (das wartet
  auf die Pflege-Sitzung Karte 09 „App-SW-Koexistenz + Tablet-
  Sichtkontrolle" vor der zweiten Bau-Iteration).

**Was nicht geändert wurde (Briefing-Verbote eingehalten):**

- **Kein Eingriff in `src/modules/01_storage.js`** — DB-Version 3
  und `sbkim_hetero_outbox`-Store-Anmeldung sind bereits aus Pflege
  Bau 06.1 da; Modul 08 nutzt nur `SbkimStorage.{init, get, put, del,
  all}`.
- **Kein Eingriff in `src/modules/05_anastomose.js`** — Modul 05
  ist seit Spec-Sitzung 06 unangetastet (`heterokaryosisOptIn` ist
  Karte-01-Co-Schreiber-Konvention, nicht Karte-05-API).
- **Kein Eingriff in `src/modules/06_heterokaryose.js`** — der
...
- **Kein `SbkimEmbedding`-Import oder -Aufruf** — Vektor-Erzeugung
  ist Aufrufer-Pflicht (Karte 08 § Verantwortlichkeiten).
...
- **Keine §0-/§1-Modul-01/05/06/07/09-/§2-/§3-/§4-/§5-Änderung** in
  INTERFACES.md — §1 Modul 08 bleibt der Spec-Sitzung-08-Vertrag.
- **Keine Karte-01-/-05-/-06-/-07-/-09-/-10-/-11-/-12-/-14-Änderung.**
- **Kein Hauptversions-Sprung** (PROTOCOL_VERSION bleibt "0.1";
  `sbkim_hetero_outbox`-Schema additiv aus Spec-Sitzung 08).
- **Keine Sage-Page-(`index.html`)-Änderung** — datengetrieben aus
  `status.json` (Karte 4 Module-Bento + Karte 14 Bau-Puls + Pie
...
- **Bau-Sitzung Modul 09 zweite Iteration mit Klaus am Live-
  Andock-Versuch** — Haupt-Pfad, **nicht headless**, wartet auf die
  Pflege Karte 09.
...
Übergabeprotokoll:
[2026-05-15 Bau-Sitzung 08 UI-Demo](sessions/archiv/2026-05-15_bau-08-ui-demo.md).
...
- **Karte 08 (`docs/components/08_ui_demo.md`) vollständig gefüllt**
  — von 134 Zeilen Werkstatt-Stub auf ~480 Zeilen volle Spec (Stil
  analog Spec-Sitzungen 00 / 06 / 07). Alle Pflichtblöcke vorhanden:
  Status-Block 🟨 Spec fertig, Im-Mycel-Bild (Pflege-Tisch am
  Mycel-Rand · sichtbare Pflege vs. versteckte Status-Geste in
  Modul 00), Mermaid-Flowchart (Klaus → UI → SbkimUiDemo → Stores →
  Modul 06 → Peer), Zweck (zwei Stellen: Outbox-Befüllung +
  Opt-In-Flag), § Modul-08-Rollenwahl (Variante b verbindlich),
  Verantwortlichkeiten Macht / Macht nicht, Schnittstelle
  (Fünf-Funktionen-API), Selbstcheck, Konfigurationswerte
  (`HETERO_OUTBOX_MAX_ENTRIES` aus §0, modul-lokal
  `OUTBOX_LABEL_MAX_LEN = 64`), Datenformate (`sbkim_hetero_outbox`-
  Wert, `addOutboxAnchor`/`setSiblingHeteroOptIn`-Argument-Form,
  Co-Schreiber-Verhalten auf `sbkim_siblings`), Fehlertabelle (16
  Lagen + sechs benannte Error-Klassen), Manueller Test
  (sieben-Punkte-Skizze für Bau-Sitzung 08), Risiken (Privacy-Leak
  via Outbox-Label, Anker-Vergiftung → Modul 10, kein Self-Apoptose-
  Knopf bewusst, `setSiblingHeteroOptIn` setzt nicht initial,
  DOM-Form offen, Quota-Frühwarnung an Modul 00 delegiert, Outbox-
  Lese-Pfad in Modul 06 als Folge-Pflege Bau 06.1 offen),
  Bauzustand-Tabelle mit „Spec gefüllt 2026-05-15"-Zeile,
  Querverweise (Modul 01 Pflicht, 02/03 indirekt, 06 Leser,
  Folge-Pflege Bau 06.1 offen, Spec-Sitzung 08.2 optional).
- **`docs/INTERFACES.md` §0 erweitert** um
  `HETERO_OUTBOX_MAX_ENTRIES = 5` (additiv, kein Hauptversions-
  Sprung; Begründung: konsistent mit `HETERO_MAX_ANCHORS = 5` —
  was in der Outbox steht, geht beim nächsten Pull raus; größere
  Outbox hätte unerreichbare Anker zur Folge).
- **`docs/INTERFACES.md` §1 Modul 01 Storage-Block erweitert**:
  neuer Store `sbkim_hetero_outbox` (Schlüssel `label` string ≤ 64
  Zeichen, Wert `{label, vector, addedAt}`, Schreiber 08, Leser 06);
  DB-Version 2 → 3 (additive Migration v=3); `sbkim_siblings`-Zeile
  um Co-Schreiber-Hinweis 08 erweitert (Schreiber bleibt Modul 05;
  Modul 08 darf AUSSCHLIESSLICH das eine additive Feld
  `heterokaryosisOptIn` setzen, wenn der Eintrag bereits existiert
  — sonst `UnknownSiblingError`); Schema-Hinweis-Block um
  Co-Schreiber-Konvention und um `sbkim_hetero_outbox`-Verhalten
  erweitert (Reihenfolge absteigend nach `addedAt` in `listOutbox`,
  Überschreib-Verhalten bei doppeltem Label, `OutboxFullError` ohne
  automatisches Verdrängen, fail-soft-Lese-Recht für Modul 06).
- **`docs/INTERFACES.md` §1 Modul 08 auf Status `entwurf`** mit
  voller Vertrag-Sektion (API-Signaturen, Nutzt-Block,
  Storage-Block, Selbstcheck-Format, Versionierungs-Vertrag,
  Fehlerverhalten mit sechs benannten Error-Klassen, Datenformate-
  Verweis auf Karte 08, Garantien für Modul 06 / 09).
- **`docs/INTERFACES.md` §6 Änderungsprotokoll-Zeile** für
  Spec-Sitzung 08 am Ende der Tabelle.
- **`docs/components/01_storage.md` § Stores + § Schema-Hinweise +
  § Versionsmigration (v=3-Zeile) + § Konfigurationswerte
  (`DB_VERSION = 3`) + § Bauzustand „Pflege Spec 08 Outbox-Anmeldung"-
  Zeile** nachgezogen. **Keine JS-Code-Änderung in
  `src/modules/01_storage.js`** — `DB_VERSION` und `STORES_V3` zieht
  die Bau-Sitzung 08 nach (Spec-Sitzung 08 spezifiziert nur den
  Vertrag).
- **`status.json` Modul 08** von `score:"werkstatt"` /
  `siegel:"in Werkstatt"` auf `score:"spec"` / `siegel:"Spec fertig"`
  hochgestuft; `abhaengig` auf `["01"]` gesetzt; `kurz` aktualisiert
  (Endknoten-Pflege-UI für Outbox + Opt-In, fünf Funktionen, neuer
  v=3-Store, Self-Apoptose-Knopf bewusst nicht in Erst-Spec).
  `config.HETERO_OUTBOX_MAX_ENTRIES = 5` am Ende des `config`-Blocks
  ergänzt; `lastUpdated` auf `2026-05-15`.
- **`scripts/update_puls_pie.py` aufgerufen** (Pflicht):
  Werkstatt 1 → 0, Spec fertig 1 → 2, Schablone 4 → 4, Code-Stub
  8 → 8, Fertig 0 → 0. 14 Module gesamt unverändert.
- **PULS-Schnellüberblicks-Zeile Modul 08** von „leere Schablone"
  auf „Spec fertig (2026-05-15)" mit erweitertem Anmerkungs-Text.
  § Als nächstes ✨ Modul 08 aus „In Arbeit (fortsetzen, nicht neu
  starten)"-Bucket entfernt und in „Spec frisch, Bau ausstehend"-
  Bucket als zweite Zeile aufgenommen (parallel zu Modul 09).

**Pflicht-Spec-Entscheidungen (alle entscheidbar — keine Blockade):**

1. **Modul-08-Rollenwahl: Variante (b) verbindlich.** Karte 08
   spezifiziert das Endknoten-`SbkimUiDemo`-JS-Modul; die Werkstatt
   `tests/manual_check.html` ist ausdrücklich KEIN Modul-08-Code
   und bleibt unbenannt. Begründung: Werkstatt hat keinen
   Endknoten-Vertrag (wird nie kopiert); Karten-Statuscodes formal
   für JS-Module gedacht; Rolle (b) schließt die Spec-Lücke aus
   Spec-Sitzung 06 (Outbox-Schreiber + Opt-In-Setter im Endknoten-UI).
2. **Schnittstelle: fünf Funktionen.** `init(options)`,
   `listOutbox()`, `addOutboxAnchor(label, vector)`,
   `removeOutboxAnchor(label)`, `setSiblingHeteroOptIn(peerNodeId,
   optIn)`. Begründung Co-Schreiber-Status auf `sbkim_siblings`:
   das eine Feld `heterokaryosisOptIn` ist Klaus-gesetzt im
   Endknoten-UI (Karte 06 § Verantwortlichkeiten); Modul 05 hat es
   in seiner Schreiber-Disziplin bewusst nicht spezifiziert.
   Karte-01-Vertragserweiterung, keine Karte-05-API-Änderung.
3. **Outbox-Store-Form:** Schlüssel `label` (string ≤ 64 Zeichen),
   Wert `{label, vector, addedAt}`, max. 5 Einträge
   (`HETERO_OUTBOX_MAX_ENTRIES`, konsistent mit
   `HETERO_MAX_ANCHORS`), Reihenfolge **absteigend nach `addedAt`**
   (neueste zuerst — UI zeigt das gerade Gepflegte oben, Modul 06
   nimmt beim Pull die frischesten Anker). Doppelte Labels
   überschreiben (Klaus pflegt einen einzigen Eintrag pro
   Stichwort). `OutboxFullError` ohne automatisches Verdrängen
   (stille Datenlöschung ohne Klaus-Bestätigung wäre
   inakzeptabel).
4. **Self-Apoptose-Knopf NICHT in Modul 08 in dieser Erst-Spec.**
   Spec-Sitzung 00 hatte ihn aus Modul 00 ausgelagert mit dem
   Hinweis „Modul 08 oder separater Endknoten-Pfad". Spec-Sitzung
   08 entscheidet: auch hier nicht — Self-Apoptose ist
   sicherheitskritisch (zweistufig + 60 s Token, Karte 07); ein
   versteckter Knopf in der Pflege-UI macht das Risiko nicht
   kleiner. Eigene Spec-Sitzung 08.2 oder Karte 07 § UI-Doku darf
   das später nachholen, *wenn* Klaus einen sichtbaren Pfad
   braucht.
5. **Doku-Fenster-Integration: eigener Endknoten-Anker.** Modul 08
   erreicht Klaus über einen klar benannten Endknoten-Pfad (z.B.
   „Einstellungen → SBKIM-Pflege"), nicht über die 5-Klick-Geste
   in Modul 00 — Modul 00 ist versteckt und nur Lese-/Trigger,
   Modul 08 schreibt und braucht sichtbare UI. DOM-Form bleibt
   offen (jeder Endknoten gestaltet sie selbst).
6. **Storage-Vertrag-Erweiterung:** v=2 → v=3 additiv;
   `sbkim_hetero_outbox` als v=3-Store; `sbkim_siblings`-Co-
   Schreiber-Hinweis in §1 Modul 01.
7. **Modul-06-Vertrags-Berührung: keine.** Karte 06 § Anker-Quelle
   erwähnt den Outbox-Store seit Spec-Sitzung 06; Modul 06 hat
   das fail-soft-Lese-Recht. Eine **Folge-Pflege Bau 06.1**
   (Outbox-Lese-Pfad in `06_heterokaryose.js`) ist nach Spec-Sitzung
   08 fällig, aber **NICHT Teil dieser Spec-Sitzung** — Karte 08
   § Querverweise und § Risiken dokumentieren das.
8. **Selbstcheck-Zeile:** synchron beim Skript-Laden,
   `MODUL 08 UI-DEMO bereit, Funktionen: init/listOutbox/
   addOutboxAnchor/removeOutboxAnchor/setSiblingHeteroOptIn`.
9. **`HETERO_OUTBOX_MAX_ENTRIES` in §0** (nicht modul-lokal in
   Karte 08), konsistent mit `HETERO_MAX_ANCHORS` /
   `SIBLING_MAX_AGE_MS` / `PROVIDER_MIN_MATCH`. `status.json.config`
   zieht mit.

**Was nicht geändert wurde (Spec-Disziplin):**

- **Kein JS-Code in `src/modules/08_ui_demo.js`** — Bau-Sitzung 08
  ist eigene Phase.
- **Keine `tests/manual_check.html`-Änderung** — Panel 08 entsteht
  in Bau-Sitzung 08.
- **Keine Karte-05-Schnittstellen-Änderung** — Co-Schreiber-Status
  von Modul 08 ist Karte-01-Vertragserweiterung, nicht Karte-05-
  API. `heterokaryosisOptIn` bleibt für Modul 05 außerhalb seiner
  Schreiber-Disziplin.
- **Keine Karte-06-Schnittstellen-Änderung** — fail-soft-Lese-Recht
  ist seit Spec-Sitzung 06 spezifiziert. Outbox-Lese-Pfad-
  Implementierung ist Folge-Pflege Bau 06.1.
- **Keine Karte-07-Schnittstellen-Änderung** — Self-Apoptose bleibt
  zweistufig + 60 s Token (Karte 07 § Schnittstelle unangetastet).
- **Keine Karten-10/11/12/14-Änderung** — Schutz-/Diffusion-
  Backlog-Stubs unangetastet.
- **Keine §2 / §3 / §4 / §5-Änderung in INTERFACES.md** — kein
  neues Datenformat (Modul 08 macht keine Netz-Aufrufe), kein
  neuer Endpunkt-Pfad, keine Versions-Regel-Änderung, keine
  Status-Farb-Mapping-Änderung.
- **Kein Hauptversions-Sprung** — `PROTOCOL_VERSION` bleibt `"0.1"`;
  `HETERO_OUTBOX_MAX_ENTRIES` additiv in §0; `sbkim_hetero_outbox`
  additiv als v=3-Store; `heterokaryosisOptIn`-Co-Schreiber additiv
  im Storage-Vertrag; Modul 08 nicht protokoll-aktiv.
- **Keine Sage-Page-(`index.html`)-Änderung** — datengetrieben aus
  `status.json` (Werkstatt 1 → 0, Spec fertig 1 → 2 reflektiert
  sich automatisch im Modul-Bento und in der Bau-Puls-Karte).

**Was offen blieb:**

- **Folge-Pflege Bau 06.1 (Outbox-Lese-Pfad in
  `src/modules/06_heterokaryose.js`)** ist nach Spec-Sitzung 08
  fällig. Heute liefert Modul 06 nur den Spore-Single-Anker-
  Fallback (Bau-Iteration 06 / 2026-05-15); mit dem Outbox-Lese-
  Pfad würde Panel 06 Test 9 (`HETERO_MAX_ANCHORS`-Begrenzung)
  voll testbar (heute teil-abgedeckt). *Headless möglich*, kleine
  Pflege. **NICHT** Teil dieser Spec-Sitzung 08.
- **Bau-Sitzung 08** (Endknoten-Code für `SbkimUiDemo` + Panel 08
  in `tests/manual_check.html`) steht aus. Spec liegt vollständig
  vor. *Headless möglich.*
- **Spec-Sitzung 08.2 (optional)** — Self-Apoptose-Knopf in Modul
  08 oder einem separaten Endknoten-Pfad. Nicht dringlich (Panel 07
  Test 6 deckt Self-Apoptose schon im Werkstatt-Sichttest ab).
- **Pflege-Sitzung „Karte 09 Schritt 10 Modul 08 im Andock-Pfad"** —
  optionale Andock-Anleitung-Erweiterung; Endknoten ruft
  `SbkimUiDemo.init()` nach `SbkimDoku.init()`. Niedrige Dringlichkeit
  (Modul 08 ist optional im Andock-Workflow).
...
3. **Bau-Sitzung Modul 09 zweite Iteration** mit Klaus am Live-
   ...
   Zeit am Tablet hat. **Nicht headless.** Wartet weiter auf
   Pflege Karte 09 App-SW-Koexistenz + Tablet-Sichtkontrolle.

---

### 2026-05-15 · Bau-Sitzung · Modul 06 Heterokaryose Code-Stub

**Getan:**

- **`src/modules/06_heterokaryose.js` geschrieben** als IIFE mit
  `window.SbkimHeterokaryose`, fünf öffentliche Funktionen
  (`init/requestHeterokaryosis/receiveHeterokaryosis/listHeterokaryosis/
  forgetHeterokaryosis`), fünf benannte Error-Klassen
  (`HeterokaryoseDependenciesError`, `UnknownSiblingError`,
  `HeterokaryoseTimeoutError`, `HeterokaryoseNetworkError`,
  `HeterokaryoseSignatureInvalidError`). Synchroner Selbstcheck beim
  Skript-Laden. `node --check` grün.
- **`src/sbkim-sw.js` um dritten fetch-Listener-Pfad
  `/sbkim/heterokaryosis`** erweitert (Message-Typ
  `SBKIM_HETEROKARYOSIS_REQUEST`, Body-Schutz 405/415/413/503 identisch
  zu 05/07). Header-Kommentar auf drei Pfade aktualisiert.
  `SBKIM_SW_STANDALONE`-Schalter aus Pflege App-SW-Koexistenz
  unangetastet. `node --check` grün.
- **`src/modules/01_storage.js` `DB_VERSION` 1 → 2** (additive
  Migration); `STORES_V2 = ["sbkim_hetero_inbox"]` als neuer Block in
  `applyMigration`. Bestehende PWAs bekommen den Store beim nächsten
  Lade additiv ohne Datenverlust. `node --check` grün.
- **`src/modules/07_apoptose.js` `CLEANUP_ORDER`** um
  `HETERO_INBOX_STORE` zwischen `INBOX_STORE` und `SPORE_STORE`
  erweitert (Position 4, vor der Identitäts-Schicht). `confirmSelfApoptose`
  löscht jetzt sechs Stores statt fünf, gefolgt von
  `resetIdentityCache()`. `node --check` grün.
- **`tests/manual_check.html` Panel 06** mit 14 Knöpfen gefüllt
  (Setup + 12 Test-Punkte aus Karte 06 § Manueller Test + Selbstcheck-
  Hinweis). Knopf-Stil exakt wie Panel 07; `SbkimUI.setStatus` für
  Pass-Check, JSON-Antworten als `output.textContent`. Alle 9 Inline-
  `<script>`-Blöcke syntaktisch validiert.
- **Karte 01** § Stores um `sbkim_hetero_inbox`-Zeile erweitert
  (Schreiber 06, Komposit-Schlüssel-Form-Hinweis); Schema-Hinweise-
  Block (additives `heterokaryosisOptIn` auf `sbkim_siblings`,
  `sbkim_anastomosis_log` zwei Schreiber); § Versionsmigration v=1/v=2-
  Tabelle; § Bauzustand „Pflege Bau 06 Store-Anmeldung"-Zeile.
- **Karte 06** Hero-Badge auf 🟦 Code-Stub; Datei-Zeile ohne
  „ausstehend"; § Bauzustand „Code geschrieben"-Zeile mit Datum +
  acht Bau-Pflicht-Entscheidungen + Test-Brücken-Surface + Anker-
  Quelle-Notiz; § Manueller Test Punkt 9 als „teil-abgedeckt" markiert.
- **Karte 07** § Schnittstelle (`confirmSelfApoptose`-Block) + §
  Apoptose-Pfad Sender-Seite Schritt 5 (Cleanup-Reihenfolge) um
  neuen `sbkim_hetero_inbox`-Cleanup-Schritt erweitert; § Bauzustand
  „Pflege Cleanup-Reihenfolge Bau 06"-Zeile.
- **INTERFACES.md** §1 Modul 01 § Storage um Schreiber-Spalten (06 für
  log/hetero-inbox), additive `heterokaryosisOptIn`-Erweiterung,
  Komposit-Schlüssel-Notiz und Schema-Hinweise-Block; §1 Modul 06
  Storage-Block um Bau-Iteration-Anker-Quelle-Notiz, Geprüft-Block
  zusätzlich „2026-05-15 (Bau-Sitzung 06)"; §1 Modul 07 § Storage +
  § Cleanup-Reihenfolge sechs Schritte plus `resetIdentityCache`; §6
  Änderungsprotokoll-Zeile „Bau-Sitzung 06" am Ende (unter
  Spec-Sitzung 06).
- **`status.json`** Modul 06 `spec` → `stub`, `siegel` „Spec fertig"
  → „Code-Stub", `kurz` aktualisiert (Bau-Stand). `lastUpdated`
  bleibt `2026-05-15`.
- **`scripts/update_puls_pie.py`** aufgerufen (Pflicht, weil
  `status.json.modules` sich geändert hat). Pie regeneriert:
  Schablone 4 → 4, Werkstatt 1 → 1, **Spec fertig 2 → 1**,
  **Code-Stub 7 → 8**, Fertig 0 → 0; 14 Module gesamt unverändert.
- **PULS-Schnellüberblicks-Zeile Modul 06** auf „Spec fertig
  (2026-05-15) / Code-Stub (2026-05-15)" hochgestuft; § Als nächstes ✨
  Modul 06 aus „Spec frisch, Bau ausstehend"-Bucket entfernt und in
  „Code-Stub frisch, Sichttest ausstehend"-Bucket aufgenommen (parallel
  zu 05/07/00).
- **Übergabeprotokoll** `docs/sessions/archiv/2026-05-15_bau-06-heterokaryose.md`
  angelegt (Format wie `2026-05-14_bau-07-apoptose.md`).

**Bau-Pflicht-Entscheidungen (acht):**

1. **Sign/Verify-Pfad als vierter Pfad bewusst dupliziert** (kein
   Re-Import) — Single-File-PWA-Stil, Konvention seit Bau-Sitzung 07.
2. **Test-Brücken-Surface:** `_invokeReceiveHeterokaryosisDirect`,
   `_buildSignedHeterokaryosisRequest`, `_verifyResponseSignature`,
   `_addPseudoSibling` (mit `heterokaryosisOptIn`-Flag-Argument —
   schreibt direkt in `sbkim_siblings`, weil Modul 06 Storage-basiert
   liest, nicht in-memory wie Modul 07), `_clearPseudoSiblings`,
   `_setReceiverHttpStatus(status|null)` (fetch-Override für 404-Test
   ohne Netz), plus `_canonicalize`/`_base64urlEncode`/
   `_base64urlDecode`/`_signEnvelope`/`_verifyEnvelope`.
3. **Service-Worker** dritter fetch-Listener-Pfad parallel zu
   `/sbkim/anastomosis` + `/sbkim/legacy`, gleicher Body-Schutz.
4. **Storage** `DB_VERSION` 1 → 2 (additive Migration), `STORES_V2`-
   Block in `applyMigration`. Bestehende PWAs bekommen den Store
   additiv beim nächsten Lade.
5. **Apoptose-Cleanup-Reihenfolge** um `sbkim_hetero_inbox` zwischen
   `sbkim_legacy_inbox` und `sbkim_spore` erweitert (vor der
   Identitäts-Schicht). Pass-Check Panel 07 Test 6 ist nach diesem
   Schritt nicht mehr 1:1 mit dem vorigen Zustand kompatibel — Folge-
   Sichttest braucht eine kleine Panel-07-Test-6-Anpassung (eine
   weitere Store-Leer-Prüfung).
6. **Synchroner Selbstcheck** beim Skript-Laden mit der vollen
   Funktions-Zeile.
7. **Anker-Quelle:** in dieser Bau-Iteration **ausschließlich** der
   Spore-Single-Anker-Fallback — `getOwnSpore()` lesen, wenn
   `domainVector` vorhanden → 1 Anker `(domain)`; sonst leeres
   `anchors:[]` mit `outcome:"shared"` (**Degraded-Modus**, weil
   Sibling-/Opt-In-Filter passiert wurde). Kein `sbkim_hetero_outbox`-
   Lese-Code, kein Stub-Anlegen, kein Store-Register.
8. **Panel 06** mit 14 Knöpfen, Knopf-Stil exakt wie Panel 07; Test 9
   (`HETERO_MAX_ANCHORS`-Begrenzung) als teil-abgedeckt markiert.

**Was nicht geändert wurde (bewusst):**

- **Keine Karte-06-Spec-Änderung** und **keine INTERFACES.md §1
  Modul 06-Schnittstellen-Änderung** — Spec ist fest seit Spec-Sitzung
  06 (PR #34, 2026-05-15). Nur additive Karten-Notiz an § Manueller
  Test Punkt 9 (Degraded-Modus-Vermerk).
- **Keine Karte-05-Schnittstellen-Änderung.** `heterokaryosisOptIn` ist
  Storage-Schema-Erweiterung, kein Modul-05-API-Eingriff.
- **Keine `sbkim_hetero_outbox`-Implementierung** — Spec-Sitzung 08
  oder Pflege Modul 02. Kein Stub-Store, kein Register.
...
- **Sage-Page (`index.html`) unangetastet** — Karte 4 (Module-Bento),
  Karte 14 (Bau-Puls) und Pie-Center-Zahl ziehen datengetrieben aus
  `status.json` nach.

**Offen / Folge-Sitzungen:**

- **Klaus' Sichttest Panel 06 im Browser** (nicht headless) sobald
  Tablet-Zeit da ist — analog Klaus' Sichttests Modul 00/05/07 vom
  2026-05-15.
- **Klaus' Re-Sichttest Panel 07 Test 6** (Self-Apoptose, jetzt mit
  sechs leeren Stores statt fünf) — Pass-Check-Block in Panel 07 Test
  6 noch additiv um `hetero_inbox_leer`-Feld erweitern; eigene Mini-
  Pflege-Sitzung oder im Rahmen der nächsten Sichttest-Runde.
- **Spec-Sitzung Modul 08 (UI-Demo)** — Werkstatt-Stub füllen, plus
  `sbkim_hetero_outbox`-Spec ergänzen (mehr als ein Anker pro Pull,
  voller `HETERO_MAX_ANCHORS`-Begrenzungs-Test in Panel 06).

**Nächster Schritt:** Bau-Sitzung Modul 09 zweite Iteration mit Klaus
am Live-Andock-Versuch bleibt der empfohlene Haupt-Pfad sobald Klaus
Zeit am Tablet hat (nach Pflege Karte 09 App-SW-Koexistenz +
Tablet-Sichtkontrolle — siehe offene Querschnitts-Frage). Headless
sinnvoll: Mini-Pflege „Karte 14 Status-Zeile" (zwei Zeilen),
PULS-Archivierung (überfällig, > 400 Zeilen), oder Spec-Sitzung
Modul 08 (UI-Demo + `sbkim_hetero_outbox`-Spec). Details und
Übergabe-Befunde im
[Übergabeprotokoll 2026-05-15 Bau-06-Heterokaryose](sessions/archiv/2026-05-15_bau-06-heterokaryose.md).

---

### 2026-05-15 · Spec-Sitzung · Modul 06 Heterokaryose gefüllt

**Getan:**

- **`docs/components/06_heterokaryose.md` vollständig gefüllt** (Stil
  analog Spec-Sitzungen 05 + 07): Status-Block 🟨 Spec fertig, Im-
  Mycel-Bild, Visualisierung (Mermaid-Sequenz-Diagramm Pull-Pattern
  mit alt/else für drei Outcomes — shared/opt-out/rejected), Zweck
  („Domain-Anker statt einzelner Domain-Vektor" als Spec-Wille),
  Verantwortlichkeiten (Macht / Macht nicht), Schnittstelle (fünf
  Funktionen), Selbstcheck, Konfigurationswerte, Anker-Quelle-Block,
  Datenformate (HeterokaryosisRequest 7 Pflichtfelder inkl. `toNodeId`
  als Pflicht; HeterokaryosisResponse 8+2 Felder mit `anchors` als
  optionalem Pflicht-bei-shared-Feld; `sbkim_hetero_inbox`-Wert mit
  Komposit-Schlüssel `peerNodeId|ts` als Drift-Spur; additives
  `heterokaryosisOptIn`-Feld auf `sbkim_siblings`; additive
  outcome-Werte in `sbkim_anastomosis_log`), Heterokaryose-Pfad in 14
  Schritten (Sender 1–6, Empfänger 7–11, Sender 12–14), Service-
  Worker-Hinweis (Variante A Page-Hosted verbindlich, gleicher
  fetch-Listener wie 05/07, MessageChannel-Brücke), Fehlertabelle mit
  12 Lagen (Outcome vs. Throw klar getrennt), Hinweis an Karte 07
  für Self-Apoptose-Cleanup (eigene Folge-Pflege), Manueller Test mit
  13 Punkten (lokaler Pull-Round-Trip · opt-out Empfänger · opt-out
  lokal · unbekannter Sibling · Sender-kein-Geschwister · `toNodeId`-
  Mismatch · Versions-Mismatch · Signatur-Manipulation ·
  `HETERO_MAX_ANCHORS`-Begrenzung · `listHeterokaryosis` ·
  `forgetHeterokaryosis` · `endpoint_unsupported` · Selbstcheck),
  Risiken-Block mit 8 Punkten (Anker-Vergiftung → 10, Privacy-Leak
  via Outbox, Replay → 11, Rate-Limit → 11, Blocklist → 12, Drift
  als Feature, Sibling-Schema-Erweiterung additiv, Anker-Quelle
  minimal in Erst-Spec), Bauzustand-Tabelle (Spec gefüllt 2026-05-15
  als erste neue Zeile), Querverweise.
- **`docs/INTERFACES.md` §1 Modul 06 gespiegelt** auf Status
  `entwurf`: Bietet-Block (Fünf-Funktionen-API), Nutzt-Block
  (`SbkimStorage` für drei Stores, `SbkimSpore` für Identität und
  Verifikation, WebCrypto via Modul 02, fetch + AbortController),
  Storage-Block (`sbkim_hetero_inbox` als angekündigter Store für
  Karte 01-Pflege in Bau-Sitzung 06; `sbkim_siblings` als Leser
  mit additiver `heterokaryosisOptIn`-Schema-Erweiterung;
  `sbkim_anastomosis_log` mit additivem outcome-Vokabular), Events
  (keine — MessageChannel `SBKIM_HETEROKARYOSIS_REQUEST`),
  Selbstcheck-Format-Zeile, Versionierungs- und Heterokaryose-
  Vertrag mit sechs Punkten (Hauptversion-Abbruch, HTTP-404 ohne
  Throw, Sibling-Filter, Opt-In beidseits, Anker-Anzahl, keine
  Weiterleitung), Fehlerverhalten-Liste mit 14 Lagen, Datenformate-
  Verweis auf §2, Service-Worker-Vertrag (Variante A verbindlich),
  Garantien für 07/08/10/11/12/14 (sechs Punkte). **§0 um
  `HETERO_MAX_ANCHORS = 5` ergänzt** (additiv, kein
  Hauptversions-Sprung; konsistent mit `SIBLING_MAX_AGE_MS` aus
  Spec-Sitzung 07). **§2 „Heterokaryose (Pull)" neuer Sub-Abschnitt**
  mit HeterokaryosisRequest (7 Pflichtfelder), HeterokaryosisResponse
  (8 Pflichtfelder + 2 optionale `anchors`/`reason`), Anker-Form
  (`{label, vector[384]}` ohne Eigen-Signatur), Versionierungs-Regel
  (additiv, neue `outcome`-Werte additiv), Verifikations-Pfad in 8
  Schritten (Form → Spore → Hauptversion → Signatur → `toNodeId` →
  Sibling-Filter → Opt-In-Filter → Antwort). **§3 unangetastet**
  (`heterokaryosis: /sbkim/heterokaryosis` ist seit Hauptsitzung
  Site-Echo eingetragen — Vertrag wurde jetzt gefüllt). **§6
  Änderungsprotokoll-Zeile am Ende** ergänzt.
- **`status.json` Modul 06** von `score:"schablone"` /
  `siegel:"noch nicht gebaut"` auf `score:"spec"` /
  `siegel:"Spec fertig"`. `kurz` aktualisiert. `config.
  HETERO_MAX_ANCHORS = 5` neu (am Ende, konsistent mit
  Reihenfolge in §0).
- **`scripts/update_puls_pie.py` aufgerufen** (Pflicht, weil
  `status.json.modules` sich geändert hat). Pie regeneriert:
  **Schablone 5 → 4**, Werkstatt 1 → 1, **Spec fertig 1 → 2**,
  Code-Stub 7 → 7, Fertig 0 → 0; 14 Module gesamt unverändert.
- **PULS-Schnellüberblicks-Zeile Modul 06** vom „leere Schablone"-
  Eintrag auf den vollen Spec-Eintrag umgestellt (Pull-Pattern,
  Opt-In beidseits, fünf Funktionen, neuer Store, kanonisch wie
  05/07).
- **PULS § Als nächstes ✨ „Spec frisch, Bau ausstehend"-Bucket**
  um Modul 06 erweitert (parallel zu Modul 09). Headless-Markierung
  klargestellt — Bau-Sitzung 06 läuft mit lokalen Pseudo-
  Geschwistern in Panel 06 (analog Bau-Sitzung 05/07).
- **Übergabeprotokoll**
  `docs/sessions/archiv/2026-05-15_spec-06-heterokaryose.md`
  angelegt (Format wie `2026-05-14_spec-05-anastomose.md`).

**Was nicht geändert wurde (bewusst):**

- **Kein JS-Code in `src/`.** `src/modules/06_heterokaryose.js`
  existiert nicht und bleibt nicht-existent bis zur Bau-Sitzung 06.
- **Karte 05 (Anastomose) unangetastet.** `heterokaryosisOptIn` als
  additives Feld auf `sbkim_siblings`-Einträgen ist eine
  Storage-Schema-Erweiterung — Modul 05 muss das Feld nicht setzen,
  Modul 06 liest fail-soft. Karte 05 § Schnittstelle bleibt
  fünf-funktional, `HandshakeRequest`/`HandshakeResponse` unverändert.
- **Karte 07 (Apoptose) unangetastet.** Self-Apoptose-Cleanup-
  Reihenfolge muss um `sbkim_hetero_inbox` erweitert werden, aber
  das ist eine eigene Folge-Pflege-Sitzung Karte 07 (oder Teil der
  Bau-Sitzung 06, sobald Re-Sichttest 07 fällig wird). Karte 06
  dokumentiert den Hinweis als § „Hinweis für Karte 07".
- **Karten 10/11/12 unangetastet.** Schutz-Backlog-Stubs werden nur
  als Hook-Punkte in Karte 06 § Risiken erwähnt.
- **Karte 14 (Diffusion) unangetastet.** Modul 06 ist als Lead-Pool-
  Konsument in Karte 14 § Verbindung zu anderen Karten schon
  dokumentiert; Modul 06 erwähnt umgekehrt Modul 14 nur als
  Querverweis.
- **Karte 01 (Storage) unangetastet.** Der neue Store
  `sbkim_hetero_inbox` ist in Karte 06 + §1 Modul 06 als
  „angekündigt" markiert; die Aufnahme in Karte 01's Store-Vertrag
  und in §1 Modul 01 (mit `Schreiber 06`-Vermerk) macht die
  Bau-Sitzung 06 als additive Erweiterung (DB-Version 1 → 2 in
  Modul 01, falls die Bau-Sitzung 06 das so umsetzt).
- **`tests/manual_check.html` unangetastet.** Panel 06 entsteht
  in der Bau-Sitzung 06.
- **Sage-Page (`index.html`) unangetastet.** Karte 4 (Module-Bento),
  Karte 14 (Bau-Puls) und Pie-Center-Zahl sind datengetrieben —
  die `status.json`-Hochstufung Modul 06 → `spec` reflektiert sich
  automatisch beim nächsten Reload (gelbe Zelle, Spec-fertig-
  Legende +1).
- **Kein Hauptversions-Sprung.** `HETERO_MAX_ANCHORS` ist additiv
  in §0; `heterokaryosisOptIn` ist additiv im Storage-Schema;
  `HeterokaryosisRequest`/`Response` sind neu (kein
  Optional→Pflicht-Wandel); neue `outcome`-Werte in
  `sbkim_anastomosis_log` sind additiv. `PROTOCOL_VERSION` bleibt
  `"0.1"`.
- **Kein `HETERO_PULL_TIMEOUT_MS` als eigene Konstante.** Modul 06
  nutzt `QUERY_TIMEOUT_MS` aus §0 (= 4000) — konsistent mit Modul
  05 und 07. Eine eigene Konstante wäre Duplikation ohne
  Mehrwert.
- **Kein `HETERO_MAX_PAYLOAD_BYTES` als eigene Konstante.** Die
  64 KiB-Body-Grenze aus 05/07 ist Service-Worker-Konvention, in
  Karte 06 § Service-Worker-Hinweis durch Querverweis abgedeckt.
- **Kein separater `sbkim_hetero_log`-Store.** Hetero-Outcomes
  landen additiv in `sbkim_anastomosis_log` — ein zweiter Log-Store
  wäre Verdopplung ohne Mehrwert; Modul 07's TTL-Sweep bleibt
  unangetastet, weil er nur `"established"`/`"re-handshake"`-
  outcomes liest.
...
eine Blockade-Klausel hätte auslösen können („`HeterokaryosisPayload`-
Inhalt ist ohne Klaus' Endknoten-Praxis nicht entscheidbar"). Beim
Durchgehen kein solcher Punkt — alle acht sind im Spec-Rahmen
entscheidbar:

1. **Was tauschen Geschwister?** **Domain-Anker**
   (`{label, vector[384]}`-Paare, max. 5). Begründung in vier
   Punkten: (a) semantisch reicher als der einzelne `domainVector`
   aus der Spore, (b) ermöglicht Sub-Domain-Lookups beim Empfänger,
   (c) erlaubt Drift-Erkennung über Zeit, (d) trägt keine
   personenbezogenen Daten und keine Inhaltsdaten (Rezepte/
   Cocktails). Alternativen (Anfrage-Statistiken / Spore-Aliase /
   k-anonymisierte Aggregate) sind entweder Privacy-belastet oder
   gehören in Modul 14 (Diffusion-Backlog).
2. **Push oder Pull?** **Pull**, drehbuchkonform.
3. **Endpunkt-Vertrag?** POST `/sbkim/heterokaryosis`, kanonisch
   signiert, JSON, ≤ 64 KiB. 405/415/413/503-Konvention analog
   05/07. HTTP 404 → `outcome:"endpoint_unsupported"` (Peer mit
   älterem Protokoll-Stand).
4. **Opt-In-Mechanik?** Additives Feld
   `sbkim_siblings[peerNodeId].heterokaryosisOptIn: boolean`,
   default `false`, fail-soft. Modul 05 setzt nicht, Klaus setzt
   im Endknoten-UI. Beidseits geprüft: Sender lokal,
   Empfänger serverseitig.
5. **Storage-Stores?** Neuer Store `sbkim_hetero_inbox`
   (Komposit-Schlüssel `peerNodeId|ts`, Drift-Spur). Logs additiv
   in `sbkim_anastomosis_log` (kein neuer Log-Store). Anker-
   Quelle `sbkim_hetero_outbox` als Spec-Wille vorbereitet,
   Implementierung Spec-Sitzung 08 oder Pflege Modul 02 — bis
   dahin Fallback Spore-Single-Anker.
6. **API-Form?** Fünf Funktionen (`init/requestHeterokaryosis/
   receiveHeterokaryosis/listHeterokaryosis/forgetHeterokaryosis`),
   `receiveHeterokaryosis` mit sync-Outcome wie 05/07 (kein Throw
   nach außen). Self-Apoptose-Cleanup-Hinweis in Karte 06, keine
   Karte-07-Änderung in dieser Sitzung (Spec-Disziplin).
7. **Versions-Vertrag?** Additiv, `protocolVersion: "0.1"` bleibt.
   404 → `outcome:"endpoint_unsupported"`. Neue outcome-Werte
   additiv.
8. **Sicherheits-/Drehbuch-Anker?** Empfangsmodus mit Antwortrecht
   strikt eingehalten. Modul 11 (Rate-Limit) als Hook-Punkt
   genannt, Modul 12 (Blocklist) als Filter-Hook — keine
   Implementierung, nur Verweise.

Die einzige nicht-triviale Entscheidung war Punkt 1 (was tauschen?).
Domain-Anker setzen sich gegen die Alternativen durch, weil sie das
einzige Datum sind, das **schon im Andock-Workflow erzeugt wird**
(Embedding-Vektor pro Stichwort) und ohne neue Embedding-Aufrufe
zur Verfügung steht. Anfrage-Statistiken bräuchten einen neuen Logger
im Endknoten; Spore-Aliase gehören in Modul 14; k-anonymisierte
Aggregate sind ein eigenes Feld.

**Was offen blieb:**

- **Bau-Sitzung Modul 06** steht aus. Spec liegt vollständig vor
  (Karte 06 + INTERFACES.md §1 Modul 06 + §2 Heterokaryose
  + §0 `HETERO_MAX_ANCHORS`). Die Bau-Sitzung muss zusätzlich:
  ...
  - Panel 06 in `tests/manual_check.html` mit ~13 Knöpfen
    (Setup + 12 Test-Punkte + Selbstcheck-Hinweis).
- **Anker-Quelle `sbkim_hetero_outbox`** spec-mäßig vorbereitet,
  aber implementations-technisch erst Spec-Sitzung 08 oder eine
  Pflege Modul 02. Bis dahin liefert jeder Knoten genau einen
  Anker pro Pull (Spore-Single-Anker mit Label `"(domain)"`) —
  primärer Nutzen ist Drift-Erkennung beim Domain-Vektor. Klaus
  kann das in der Bau-Sitzung 06 sichtprüfen.
- **Karte 07 Self-Apoptose-Cleanup-Erweiterung** als
  Mini-Folge-Pflege notiert. Wird beim ersten echten Re-Sichttest
  von Panel 07 nach Modul-06-Einbau spürbar (sonst tolerabel —
  `sbkim_hetero_inbox` ist ohne Identität leer-funktional).
- **`heterokaryosisOptIn`-Setter im Endknoten-UI** als Folge-
  Aufgabe für Modul 00 (Doku-Fenster) oder Modul 08 (UI-Demo).
  Modul 06 liest nur. Klaus markiert pro Geschwister im UI, mit
  wem er erweiterten Datenaustausch erlaubt.
- **Karte 14 Status-Zeile-Mini-Pflege** und **PULS-400-Zeilen-
  Konvention** weiter offen (überfällig). Diese Spec-Sitzung
  löst sie nicht.
- **Bau-Sitzung Modul 09 zweite Iteration mit Klaus am Live-
  Andock-Versuch** bleibt der empfohlene Haupt-Pfad (Karte 09 ist
  vollständig: neun Schritte + Pre-Flight + 3a/3b, `sbkim-sw.js`
  hat den `SBKIM_SW_STANDALONE`-Schalter; Modul 06 ist
  unabhängig).

**Nächster sinnvoller Schritt:**

1. **Bau-Sitzung Modul 09 zweite Iteration mit Klaus am Live-
   Andock-Versuch** (Empfehlung: bei Tablet-Zeit). **Nicht
   headless.** Hauptpfad bleibt unverändert.
2. Headless-Alternativen (in fallender Priorität):
   - **Bau-Sitzung Modul 06 Heterokaryose** — Spec liegt
     vollständig vor; lokaler Zwei-Knoten-Test im Panel 06
     analog 05/07; erweitert Karte 01 (Store) + Karte 07
     (Cleanup) + `src/sbkim-sw.js` (fetch-Listener) additiv.
   - **Mini-Folge-Pflege „Karte 14 Status-Zeile nachziehen"**
     — zwei Zeilen, *headless möglich*.
   - **Pflege „PULS-Archivierung"** — überfällig, *headless
     möglich*.
   - **Spec-Sitzung Modul 08 UI-Demo** — Werkstatt-Stub füllen,
     *headless möglich*; ergänzt den Outbox-Pfad für Modul 06.
