...
Geprüft: 2026-05-25 (Spec-Sitzung 17 — Vier-Slot-Live-Status-Dashboard
         ...
         2026-05-26 (Pflege Tooltips + Self-Heartbeat — title-Attribut
                     auf Slot-Buttons + Icon-Buttons weggelassen
                     (Doppel-Tooltips-Fix DeX-Chrome), aria-label
                     trägt vollen Text. Self-Heartbeat-Fallback in
                     Modul 17 5 s nach init() wenn alive=0 und
                     SbkimSpore.ready=true → synthetisches sbkim:alive
                     mit synthetic:true-Marker. Schema-Erweiterung
                     additiv. Headless-Smoke 28→32/32 grün. KEIN
                     Modul-02-Eingriff.),
         2026-05-26 (Pflege Sub-(e)-Visueller-Slot-Render — Folge-Pflege
                     zum Endknoten-Sichttest Sub (e). Modul 17 erweitert
                     um Stufen-Render am sichtbaren SIEGEL-Slot-Button:
                     neuer Closure-State `siegelStufeRendered`, neue
                     Helper `getCurrentSiegelStufe()` + `applySiegelStufe
                     (stufe, withAnimation)`. `mountSiegelSlot()` ruft
                     `applySiegelStufe(initial, false)` nach Mount;
                     `onHandshake()` ruft via setTimeout(0) `applySiegel
                     Stufe(getCurrentSiegelStufe(), true)` — der Trick
                     verschiebt den Lookup ans Event-Loop-Ende, damit
                     Modul 16's Listener garantiert gelaufen ist
                     (Listener-Reihenfolge: Modul 17 init VOR Modul 16
                     init im Endknoten-Andocker). CSS-Block erweitert
                     um vier neue Regeln (Bronze: filter saturate(0.5)
                     brightness(0.78) am ::before + Glyph; Gold:
                     filter:none) + neuer Keyframe sbkim-widget-siegel-
                     stufenwechsel (600 ms Skalierungs-Puls +
                     Gold-Glow). _meta um Live-Getter siegelStufeRendered
                     erweitert. Panel 17 um Knöpfe 13+14 erweitert
                     (Bronze-Initial + Bronze→Gold-Wechsel). Headless-
                     Smoke 32→34/34 grün. Regression smoke_bau15b 31/31
                     + smoke_bau16_sub_e_bronze 15/15 grün. KEIN Modul-
                     16-Eingriff, KEIN PROTOCOL_VERSION-/DB_VERSION-/
                     BACKUP_FORMAT_VERSION-Bump, KEIN ZERTIFIKAT_ASPEKTE-
                     Eintrag (Render-Schicht-Pflege).)
...
| 2026-05-26 | Pflege 17 Widget-Bronze/Gold-Render (Sub-(e)-Folge-Pflege 1/3) | Folge-Pflege zum Endknoten-Sichttest Cross-Knoten Sub (e) am 2026-05-26 (Folge-Befund 1). Klaus' Befund: visueller Bronze/Gold-Unterschied am sichtbaren SIEGEL-Slot-Button im Floating-Widget fehlt — Modul 16's `data-stufe="bronze"`/`"gold"` wirkt nur am unsichtbaren Widget-Proxy-Span im Inneren, nicht am Slot-Button. **§ 1 Modul 17 Geprüft-Zeile** um Pflege-Eintrag erweitert. **Code in `src/modules/17_floating_widget.js` additiv** (KEIN Bruch der Public-Surface, KEINE Refactoring): neuer Closure-State `siegelStufeRendered:null` + `siegelStufenwechselTimeoutId:null`; neuer Helper `getCurrentSiegelStufe()` (fail-soft Lookup auf `SbkimSiegel._meta.siegelStufe`, Default `"bronze"`, try/catch um null/missing); neuer Helper `applySiegelStufe(stufe, withAnimation)` (normalisiert Stufe-String, setzt `data-siegel-stufe`-Attribut am `slotElements.siegel`-Button + idempotent, Animation nur bei `"bronze"`→`"gold"`-Transition via `setTimeout(600)`); `mountSiegelSlot()` um EINEN Aufruf `applySiegelStufe(getCurrentSiegelStufe(), false)` nach Slot-Mount (vor First-Boot-Animation) erweitert; `onHandshake()` um EINEN Aufruf `setTimeout(0) { applySiegelStufe(getCurrentSiegelStufe(), true) }` am Ende erweitert (setTimeout(0)-Trick: verschiebt Lookup ans Event-Loop-Ende, damit Modul 16's Listener garantiert gelaufen ist — Listener-Reihenfolge `addEventListener` ist Registrierungs-Reihenfolge, Modul 17 init läuft VOR Modul 16 init im Endknoten-Andocker). **`buildCss()`-Block in Modul 17 erweitert** um vier neue Regeln: `#sbkim-widget [data-siegel-stufe="bronze"]::before` mit `filter:saturate(0.5) brightness(0.78)` + Glyph-Filter parallel; `#sbkim-widget [data-siegel-stufe="gold"]::before` mit `filter:none` + Glyph; `.sbkim-widget-siegel-stufenwechsel::before` mit Animation; neuer `@keyframes sbkim-widget-siegel-stufenwechsel` (600 ms Skalierung 1.0→1.35→1.0 mit Gold-Glow-Box-Shadow). **`_meta`** um Live-Getter `siegelStufeRendered` (string\|null) erweitert. **Panel 17** in `tests/manual_check.html` um Knöpfe 13 (Bronze-Initial-Stand am sichtbaren Slot) + 14 (synthetischer Handshake → Gold-Wechsel + Stufenwechsel-Klasse für 600 ms) erweitert. **Headless-Smoke** `tests/smoke_bau17_floating_widget.mjs` von 32 → **34 Proben** (Probe 32 Bronze-Initial + Probe 33 sbkim:handshake → Gold + Animation), 34/34 grün. **Regression** smoke_bau15b 31/31 + smoke_bau16_sub_e_bronze 15/15 grün. **node --check** für `17_floating_widget.js` + alle 13 Inline-`<script>`-Blöcke in `tests/manual_check.html` grün. **PROTOCOL_VERSION** / **DB_VERSION** / **BACKUP_FORMAT_VERSION** unverändert. **KEIN Modul-16-Eingriff** (Modul 16's Sub-(e)-Logik bleibt unangetastet — Modul 17 spiegelt nur den Stand am sichtbaren Slot). **KEIN Endknoten-Eingriff** (Endknoten ziehen den neuen Modul-17-Code in eigener Folge-PR nach — kombinierbar mit den anderen zwei Sub-(e)-Folge-Pflegen Modul-05-Update + Modal-Local-Time). **KEIN ZERTIFIKAT_ASPEKTE-Eintrag** (Render-Schicht-Pflege, kein Sicherheits-Modul-Update — CLAUDE.md § „Sicherheits-Module pflegen Aspekte" greift nicht). **`status.json` Modul 17 unverändert** (bleibt `score:"stub"`; `update_puls_pie.py` NICHT aufgerufen — additive UX-Pflege, kein Score-Wechsel). Klaus' Sub-(e)-Befund 1 von 3 ist hiermit gelöst. Brief: `docs/sessions/BRIEF_PFLEGE_17_WIDGET_BRONZE_GOLD_RENDER.md`. Übergabeprotokoll `docs/sessions/archiv/2026-05-26_pflege-17-widget-bronze-gold-render.md`. |
