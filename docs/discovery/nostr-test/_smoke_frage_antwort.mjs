...
ok("Probe 5: Schlüssel-Feld ist Passwort-Typ", /id="judgekey" type="password"/.test(html));
ok("Probe 5: Merken nur opt-in (Häkchen + persistJudge)", /id="judgeremember"/.test(html) && /function persistJudge/.test(html));
ok("Probe 5: Häkchen weg → Schlüssel gelöscht (removeItem)", /localStorage\.removeItem\(LS_JUDGE_KEY\)/.test(html));
ok("Probe 5: gemerkter Schlüssel wird beim Laden wiederhergestellt (restoreJudge)", /function restoreJudge/.test(html));
